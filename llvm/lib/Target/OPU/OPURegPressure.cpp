...
LLVM_DUMP_METHOD
void llvm::printOPULivesAt(SlotIndex SI, const LiveIntervals &LIS,
...
static bool isEqual(const OPURPTracker::LiveRegSet &S1,
...
unsigned OPURegPressure::getRegKind(unsigned Reg,
                                   const MachineRegisterInfo &MRI) {
  assert(Register::isVirtualRegister(Reg));
  const auto RC = MRI.getRegClass(Reg);
  auto STI = static_cast<const OPURegisterInfo *>(MRI.getTargetRegisterInfo());
  return STI->isSGPRClass(RC)
             ? (STI->getRegSizeInBits(*RC) == 32 ? SGPR32 : SGPR_TUPLE)
             : (STI->getRegSizeInBits(*RC) == 32 ? VGPR32 : VGPR_TUPLE);
}

void OPURegPressure::inc(unsigned Reg, LaneBitmask PrevMask, LaneBitmask NewMask,
                        const MachineRegisterInfo &MRI) {
  if (NewMask == PrevMask)
    return;
...
bool OPURegPressure::less(const OPUSubtarget &ST, const OPURegPressure &O,
...
#if !defined(NDEBUG) || defined(LLVM_ENABLE_DUMP)
LLVM_DUMP_METHOD
void OPURegPressure::print(raw_ostream &OS, const OPUSubtarget *ST) const {
  OS << "VGPRs: " << getVGPRNum();
  if (ST)
...
static LaneBitmask getUsedRegMask(const MachineOperand &MO,
  ...
  return getOPULiveLaneMask(MO.getReg(), SI, LIS, MRI);
}
...
LaneBitmask llvm::getOPULiveLaneMask(unsigned Reg, SlotIndex SI,
                                    const LiveIntervals &LIS,
                                    const MachineRegisterInfo &MRI) {
...
OPURPTracker::LiveRegSet llvm::getOPULiveRegs(SlotIndex SI,
                                            const LiveIntervals &LIS,
                                            const MachineRegisterInfo &MRI) {
  OPURPTracker::LiveRegSet LiveRegs;
  for (unsigned I = 0, E = MRI.getNumVirtRegs(); I != E; ++I) {
    auto Reg = Register::index2VirtReg(I);
    if (!LIS.hasInterval(Reg))
      continue;
    auto LiveMask = getOPULiveLaneMask(Reg, SI, LIS, MRI);
    if (LiveMask.any())
...
void OPURPTracker::reset(const MachineInstr &MI, const LiveRegSet *LiveRegsCopy,
                        bool After) {
...
void OPUUpwardRPTracker::reset(const MachineInstr &MI,
                              const LiveRegSet *LiveRegsCopy) {
  OPURPTracker::reset(MI, LiveRegsCopy, true);
}

void OPUUpwardRPTracker::recede(const MachineInstr &MI) {
...
bool OPUDownwardRPTracker::reset(const MachineInstr &MI,
                                const LiveRegSet *LiveRegsCopy) {
  ...
  if (NextMI == MBBEnd)
    return false;
  OPURPTracker::reset(*NextMI, LiveRegsCopy, false);
  return true;
}

bool OPUDownwardRPTracker::advanceBeforeNext() {
  assert(MRI && "call reset first");
...
void OPUDownwardRPTracker::advanceToNext() {
...
bool OPUDownwardRPTracker::advance() {
...
bool OPUDownwardRPTracker::advance(MachineBasicBlock::const_iterator End) {
  while (NextMI != End)
    if (!advance())
      return false;
  return true;
...
bool OPUDownwardRPTracker::advance(MachineBasicBlock::const_iterator Begin,
...
#if !defined(NDEBUG) || defined(LLVM_ENABLE_DUMP)
LLVM_DUMP_METHOD
static void reportMismatch(const OPURPTracker::LiveRegSet &LISLR,
                           const OPURPTracker::LiveRegSet &TrackedLR,
...
bool OPUUpwardRPTracker::isValid() const {
  const auto &SI = LIS.getInstructionIndex(*LastTrackedMI).getBaseIndex();
  const auto LISLR = llvm::getOPULiveRegs(SI, LIS, *MRI);

  const auto &TrackedLR = LiveRegs;

  if (!isEqual(LISLR, TrackedLR)) {
    dbgs() << "\nOPUUpwardRPTracker error: Tracked and"
              " LIS reported livesets mismatch:\n";
    printOPULivesAt(SI, LIS, *MRI);
  ...
  if (LISPressure != CurPressure) {
    dbgs() << "OPUUpwardRPTracker error: Pressure sets different\nTracked: ";
...
void OPURPTracker::printLiveRegs(raw_ostream &OS, const LiveRegSet &LiveRegs,
                                const MachineRegisterInfo &MRI) {
