//===- PPUMatInt.cpp - Immediate materialisation -------------*- C++ -*--===//
//
...
void generateInstSeq(int64_t Val, bool IsRV64, InstSeq &Res) {
  if (isInt<32>(Val)) {
    ...
    if (Hi20)
      Res.push_back(Inst(PPU::LUI, Hi20));

    if (Lo12 || Hi20 == 0) {
      unsigned AddiOpc = (IsRV64 && Hi20) ? PPU::ADDIW : PPU::ADDI;
  ...
  Res.push_back(Inst(PPU::SLLI, ShiftAmount));
  if (Lo12)
    Res.push_back(Inst(PPU::ADDI, Lo12));
}
