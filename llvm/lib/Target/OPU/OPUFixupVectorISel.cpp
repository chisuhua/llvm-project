//===-- OPUFixupVectorISel.cpp - Fixup post ISel vector issues -------------===//
//
...
//
/// \file
/// OPUFixupVectorISel pass cleans up post ISEL Vector issues.
...
class OPUFixupVectorISel : public MachineFunctionPass {
public:
  static char ID;

public:
  OPUFixupVectorISel() : MachineFunctionPass(ID) {
    initializeOPUFixupVectorISelPass(*PassRegistry::getPassRegistry());
  }
...
INITIALIZE_PASS(OPUFixupVectorISel, DEBUG_TYPE,
                "OPU Fixup Vector ISel", false, false)

char OPUFixupVectorISel::ID = 0;

char &llvm::OPUFixupVectorISelID = OPUFixupVectorISel::ID;

FunctionPass *llvm::createOPUFixupVectorISelPass() {
  return new OPUFixupVectorISel();
}
...
static bool findSRegBaseAndIndex(MachineOperand *Op,
  ...
  while (!Worklist.empty()) {
    ...
    case OPU::V_ADD_I32_e64:
      ...
      if (DefInst->getOperand(2).getSubReg() != OPU::NoSubRegister)
        continue;
      BaseReg = DefInst->getOperand(2).getReg();
      if (DefInst->getOperand(3).getSubReg() != OPU::NoSubRegister)
        continue;
      ...
      if (OPU::getRegBitWidth(IdxRC->getID()) != 64)
        continue;
      ...
      if (OPU::getRegBitWidth(BaseRC->getID()) != 64)
        continue;
...
static bool fixupGlobalSaddr(MachineBasicBlock &MBB,
  ...
  for (I = MBB.begin(); I != MBB.end(); I = Next) {
    Next = std::next(I);
    MachineInstr &MI = *I;
    int NewOpcd = OPU::getGlobalSaddrOp(MI.getOpcode());
    if (NewOpcd < 0)
      continue;
    ...
    unsigned BaseReg = 0;
    unsigned IndexReg = 0;
    MachineOperand *Op = TII->getNamedOperand(MI, OPU::OpName::vaddr);
    if (!findSRegBaseAndIndex(Op, BaseReg, IndexReg, MRI, TRI))
      continue;
    ++NumSGPRGlobalSaddrs;
    FuncModified = true;
    // Create the new _SADDR Memory instruction.
    bool HasVdst = TII->getNamedOperand(MI, OPU::OpName::vdst) != nullptr;
    MachineOperand *VData = TII->getNamedOperand(MI, OPU::OpName::vdata);
    ...
    NewGlob->addOperand(*TII->getNamedOperand(MI, OPU::OpName::offset));

    MachineOperand *Glc = TII->getNamedOperand(MI, OPU::OpName::glc);
    // Atomics dont have a GLC, so omit the field if not there.
    if (Glc)
      NewGlob->addOperand(MF, *Glc);

    MachineOperand *DLC = TII->getNamedOperand(MI, OPU::OpName::dlc);
    if (DLC)
      NewGlob->addOperand(MF, *DLC);

    NewGlob->addOperand(*TII->getNamedOperand(MI, OPU::OpName::slc));
...
bool OPUFixupVectorISel::runOnMachineFunction(MachineFunction &MF) {
