...
//
/// \file
/// This pass adds opu.uniform metadata to IR values so this information
...
#include "llvm/InitializePassses.h"
#include "llvm/Support/Debug.h"
#include "llvm/Support/raw_ostream.h"

#define DEBUG_TYPE "opu-annotate-uniform"

using namespace llvm;

namespace {

class OPUAnnotateUniformValues : public FunctionPass,
                       public InstVisitor<OPUAnnotateUniformValues> {
...
public:
  static char ID;
  OPUAnnotateUniformValues() :
    FunctionPass(ID) { }
  bool doInitialization(Module &M) override;
  bool runOnFunction(Function &F) override;
  StringRef getPassName() const override {
    return "OPU Annotate Uniform Values";
  }
...
INITIALIZE_PASS_BEGIN(OPUAnnotateUniformValues, DEBUG_TYPE,
                      "Add OPU uniform metadata", false, false)
INITIALIZE_PASS_DEPENDENCY(LegacyDivergenceAnalysis)
INITIALIZE_PASS_DEPENDENCY(MemoryDependenceWrapperPass)
INITIALIZE_PASS_DEPENDENCY(LoopInfoWrapperPass)
INITIALIZE_PASS_END(OPUAnnotateUniformValues, DEBUG_TYPE,
                    "Add OPU uniform metadata", false, false)

char OPUAnnotateUniformValues::ID = 0;

static void setUniformMetadata(Instruction *I) {
  I->setMetadata("opu.uniform", MDNode::get(I->getContext(), {}));
}
static void setNoClobberMetadata(Instruction *I) {
  I->setMetadata("opu.noclobber", MDNode::get(I->getContext(), {}));
}
...
bool OPUAnnotateUniformValues::isClobberedInFunction(LoadInst * Load) {
...
void OPUAnnotateUniformValues::visitBranchInst(BranchInst &I) {
  if (DA->isUniform(&I))
    setUniformMetadata(I.getParent()->getTerminator());
}

void OPUAnnotateUniformValues::visitLoadInst(LoadInst &I) {
  Value *Ptr = I.getPointerOperand();
  if (!DA->isUniform(Ptr))
    return;
  auto isGlobalLoad = [&](LoadInst &Load)->bool {
    return Load.getPointerAddressSpace() == OPUAS::GLOBAL_ADDRESS;
  };
...
bool OPUAnnotateUniformValues::doInitialization(Module &M) {
  return false;
}

bool OPUAnnotateUniformValues::runOnFunction(Function &F) {
  if (skipFunction(F))
    return false;
  ...
  isEntryFunc = OPU::isKernelFunction(F);
...
FunctionPass *
llvm::createOPUAnnotateUniformValues() {
  return new OPUAnnotateUniformValues();
}
