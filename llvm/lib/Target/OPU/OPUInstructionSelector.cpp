...
#include "OPUInstructionSelector.h"
...
OPUInstructionSelector::OPUInstructionSelector(
    const OPUTargetMachine &TM, const OPUSubtarget &STI,
    const OPURegisterBankInfo &RBI)
...
/*
bool OPUInstructionSelector::select(MachineInstr &I) {
...
namespace llvm {
InstructionSelector *
createOPUInstructionSelector(const OPUTargetMachine &TM,
                               OPUSubtarget &Subtarget,
                               OPURegisterBankInfo &RBI) {
  return new OPUInstructionSelector(TM, Subtarget, RBI);
}
} // end namespace llvm
...
const char *OPUInstructionSelector::getName() { return DEBUG_TYPE; }
...
static bool isSCC(Register Reg, const MachineRegisterInfo &MRI) {
  ...
  if (RC) {
    ...
    if (RC->getID() != OPU::SReg_32RegClassID)
      return false;
  ...
  return RB->getID() == OPU::SCCRegBankID;
}

bool OPUInstructionSelector::isVCC(Register Reg,
  ...
  const RegisterBank *RB = RegClassOrBank.get<const RegisterBank *>();
  return RB->getID() == OPU::VCCRegBankID;
}

bool OPUInstructionSelector::selectCOPY(MachineInstr &I) const {
  ...
  if (isVCC(DstReg, MRI)) {
    ...
    if (!isVCC(SrcReg, MRI)) {
      ...
      BuildMI(*BB, &I, DL, TII.get(OPU::V_CMP_NE_U32_e64), DstReg)
        .addImm(0)
        .addReg(SrcReg);
...
bool OPUInstructionSelector::selectPHI(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
...
MachineOperand
OPUInstructionSelector::getSubOperand64(MachineOperand &MO,
  ...
  if (MO.isReg()) {
    ...
    BuildMI(*BB, MI, MI->getDebugLoc(), TII.get(OPU::COPY), DstReg)
...
static unsigned getLogicalBitOpcode(unsigned Opc, bool Is64) {
  switch (Opc) {
  case OPU::G_AND:
    return Is64 ? OPU::S_AND_B64 : OPU::S_AND_B32;
  case OPU::G_OR:
    return Is64 ? OPU::S_OR_B64 : OPU::S_OR_B32;
  case OPU::G_XOR:
    return Is64 ? OPU::S_XOR_B64 : OPU::S_XOR_B32;
  default:
    llvm_unreachable("not a bit op");
  }
}

bool OPUInstructionSelector::selectG_AND_OR_XOR(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  MachineOperand &Dst = I.getOperand(0);
  ...
  // the result?
  if (DstRB->getID() == OPU::SPRRegBankID) {
...
bool OPUInstructionSelector::selectG_ADD_SUB(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  ...
  unsigned Size = RBI.getSizeInBits(DstReg, MRI, TRI);
  const RegisterBank *DstRB = RBI.getRegBank(DstReg, MRI, TRI);
  const bool IsSALU = DstRB->getID() == OPU::SPRRegBankID;
  const bool Sub = I.getOpcode() == TargetOpcode::G_SUB;

  if (Size == 32) {
    if (IsSALU) {
      const unsigned Opc = Sub ? OPU::S_SUB_U32 : OPU::S_ADD_U32;
      MachineInstr *Add =
    ...
    if (STI.hasAddNoCarry()) {
      const unsigned Opc = Sub ? OPU::V_SUB_U32_e64 : OPU::V_ADD_U32_e64;
      I.setDesc(TII.get(Opc));
      I.addOperand(*MF, MachineOperand::CreateImm(0));
      I.addOperand(*MF, MachineOperand::CreateReg(OPU::TMSK, false, true));
      return constrainSelectedInstRegOperands(I, TII, TRI, RBI);
    }

    const unsigned Opc = Sub ? OPU::V_SUB_I32_e64 : OPU::V_ADD_I32_e64;
  ...
  const TargetRegisterClass &RC
    = IsSALU ? OPU::SReg_64RegClass : OPU::VReg_64RegClass;
  const TargetRegisterClass &HalfRC
    = IsSALU ? OPU::SReg_32RegClass : OPU::VPR_32RegClass;

  MachineOperand Lo1(getSubOperand64(I.getOperand(1), HalfRC, OPU::sub0));
  MachineOperand Lo2(getSubOperand64(I.getOperand(2), HalfRC, OPU::sub0));
  MachineOperand Hi1(getSubOperand64(I.getOperand(1), HalfRC, OPU::sub1));
  MachineOperand Hi2(getSubOperand64(I.getOperand(2), HalfRC, OPU::sub1));

  Register DstLo = MRI.createVirtualRegister(&HalfRC);
  Register DstHi = MRI.createVirtualRegister(&HalfRC);

  if (IsSALU) {
    BuildMI(*BB, &I, DL, TII.get(OPU::S_ADD_U32), DstLo)
      .add(Lo1)
      .add(Lo2);
    BuildMI(*BB, &I, DL, TII.get(OPU::S_ADDC_U32), DstHi)
      .add(Hi1)
      .add(Hi2);
  } else {
    const TargetRegisterClass *CarryRC = TRI.getWaveMaskRegClass();
    Register CarryReg = MRI.createVirtualRegister(CarryRC);
    BuildMI(*BB, &I, DL, TII.get(OPU::V_ADD_I32_e64), DstLo)
      .addDef(CarryReg)
      .add(Lo1)
      .add(Lo2)
      .addImm(0);
    MachineInstr *Addc = BuildMI(*BB, &I, DL, TII.get(OPU::V_ADDC_U32_e64), DstHi)
  ...
  BuildMI(*BB, &I, DL, TII.get(OPU::REG_SEQUENCE), DstReg)
    .addReg(DstLo)
    .addImm(OPU::sub0)
    .addReg(DstHi)
    .addImm(OPU::sub1);
...
bool OPUInstructionSelector::selectG_EXTRACT(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
...
bool OPUInstructionSelector::selectG_MERGE_VALUES(MachineInstr &MI) const {
...
bool OPUInstructionSelector::selectG_UNMERGE_VALUES(MachineInstr &MI) const {
  MachineBasicBlock *BB = MI.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
...
bool OPUInstructionSelector::selectG_GEP(MachineInstr &I) const {
  return selectG_ADD_SUB(I);
}

bool OPUInstructionSelector::selectG_IMPLICIT_DEF(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
...
bool OPUInstructionSelector::selectG_INSERT(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
...
bool OPUInstructionSelector::selectG_INTRINSIC(MachineInstr &I) const {
  ...
  case Intrinsic::opu_if_break: {
    ...
    BuildMI(*BB, &I, I.getDebugLoc(), TII.get(OPU::SI_IF_BREAK))
      .add(I.getOperand(0))
      .add(I.getOperand(2))
      .add(I.getOperand(3));
...
static int getV_CMPOpcode(CmpInst::Predicate P, unsigned Size) {
  ...
  case CmpInst::ICMP_NE:
    return Size == 32 ? OPU::V_CMP_NE_U32_e64 : OPU::V_CMP_NE_U64_e64;
  case CmpInst::ICMP_EQ:
    return Size == 32 ? OPU::V_CMP_EQ_U32_e64 : OPU::V_CMP_EQ_U64_e64;
  case CmpInst::ICMP_SGT:
    return Size == 32 ? OPU::V_CMP_GT_I32_e64 : OPU::V_CMP_GT_I64_e64;
  case CmpInst::ICMP_SGE:
    return Size == 32 ? OPU::V_CMP_GE_I32_e64 : OPU::V_CMP_GE_I64_e64;
  case CmpInst::ICMP_SLT:
    return Size == 32 ? OPU::V_CMP_LT_I32_e64 : OPU::V_CMP_LT_I64_e64;
  case CmpInst::ICMP_SLE:
    return Size == 32 ? OPU::V_CMP_LE_I32_e64 : OPU::V_CMP_LE_I64_e64;
  case CmpInst::ICMP_UGT:
    return Size == 32 ? OPU::V_CMP_GT_U32_e64 : OPU::V_CMP_GT_U64_e64;
  case CmpInst::ICMP_UGE:
    return Size == 32 ? OPU::V_CMP_GE_U32_e64 : OPU::V_CMP_GE_U64_e64;
  case CmpInst::ICMP_ULT:
    return Size == 32 ? OPU::V_CMP_LT_U32_e64 : OPU::V_CMP_LT_U64_e64;
  case CmpInst::ICMP_ULE:
    return Size == 32 ? OPU::V_CMP_LE_U32_e64 : OPU::V_CMP_LE_U64_e64;
  }
}

int OPUInstructionSelector::getS_CMPOpcode(CmpInst::Predicate P,
                                              unsigned Size) const {
  if (Size == 64) {
    ...
    switch (P) {
    case CmpInst::ICMP_NE:
      return OPU::S_CMP_LG_U64;
    case CmpInst::ICMP_EQ:
      return OPU::S_CMP_EQ_U64;
    default:
      return -1;
    }
  }
  ...
  switch (P) {
  case CmpInst::ICMP_NE:
    return OPU::S_CMP_LG_U32;
  case CmpInst::ICMP_EQ:
    return OPU::S_CMP_EQ_U32;
  case CmpInst::ICMP_SGT:
    return OPU::S_CMP_GT_I32;
  case CmpInst::ICMP_SGE:
    return OPU::S_CMP_GE_I32;
  case CmpInst::ICMP_SLT:
    return OPU::S_CMP_LT_I32;
  case CmpInst::ICMP_SLE:
    return OPU::S_CMP_LE_I32;
  case CmpInst::ICMP_UGT:
    return OPU::S_CMP_GT_U32;
  case CmpInst::ICMP_UGE:
    return OPU::S_CMP_GE_U32;
  case CmpInst::ICMP_ULT:
    return OPU::S_CMP_LT_U32;
  case CmpInst::ICMP_ULE:
    return OPU::S_CMP_LE_U32;
  default:
    llvm_unreachable("Unknown condition code!");
  }
}

bool OPUInstructionSelector::selectG_ICMP(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  const DebugLoc &DL = I.getDebugLoc();
  ...
  if (isSCC(CCReg, MRI)) {
    ...
    MachineInstr *ICmp = BuildMI(*BB, &I, DL, TII.get(Opcode))
            .add(I.getOperand(2))
            .add(I.getOperand(3));
    BuildMI(*BB, &I, DL, TII.get(OPU::COPY), CCReg)
      .addReg(OPU::SCC);
    bool Ret =
        constrainSelectedInstRegOperands(*ICmp, TII, TRI, RBI) &&
        RBI.constrainGenericRegister(CCReg, OPU::SReg_32RegClass, MRI);
    I.eraseFromParent();
    return Ret;
  }
...
buildEXP(const TargetInstrInfo &TII, MachineInstr *Insert, unsigned Tgt,
  ...
  unsigned Opcode = Done ? OPU::EXP_DONE : OPU::EXP;
...
bool OPUInstructionSelector::selectG_INTRINSIC_W_SIDE_EFFECTS(
...
bool OPUInstructionSelector::selectG_SELECT(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  const DebugLoc &DL = I.getDebugLoc();
  ...
  if (isSCC(CCReg, MRI)) {
    unsigned SelectOpcode = Size == 64 ? OPU::S_CSELECT_B64 :
                                         OPU::S_CSELECT_B32;
    MachineInstr *CopySCC = BuildMI(*BB, &I, DL, TII.get(OPU::COPY), OPU::SCC)
            .addReg(CCReg);
  ...
  MachineInstr *Select =
      BuildMI(*BB, &I, DL, TII.get(OPU::V_CNDMASK_B32_e64), DstReg)
              .addImm(0)
              .add(I.getOperand(3))
              .addImm(0)
              .add(I.getOperand(2))
...
bool OPUInstructionSelector::selectG_STORE(MachineInstr &I) const {
  initM0(I);
  return selectImpl(I, *CoverageInfo);
}
...
static int sizeToSubRegIndex(unsigned Size) {
  ...
  case 32:
    ...
    /*
  case 64:
    return OPU::sub0_sub1;
  case 96:
    return OPU::sub0_sub1_sub2;
  case 128:
    return OPU::sub0_sub1_sub2_sub3;
  case 256:
    return OPU::sub0_sub1_sub2_sub3_sub4_sub5_sub6_sub7;
    */
  default:
    if (Size < 32)
      return OPU::sub0;
...
bool OPUInstructionSelector::selectG_TRUNC(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
...
bool OPUInstructionSelector::selectG_SZA_EXT(MachineInstr &I) const {
  bool Signed = I.getOpcode() == OPU::G_SEXT;
  const DebugLoc &DL = I.getDebugLoc();
  ...
  if (SrcBank->getID() == OPU::SCCRegBankID) {
    if (SrcTy != S1 || DstSize > 64) // Invalid
      return false;

    unsigned Opcode =
        DstSize > 32 ? OPU::S_CSELECT_B64 : OPU::S_CSELECT_B32;
    const TargetRegisterClass *DstRC =
        DstSize > 32 ? &OPU::SReg_64RegClass : &OPU::SReg_32RegClass;

    // FIXME: Create an extra copy to avoid incorrectly constraining the result
    // of the scc producer.
    Register TmpReg = MRI.createVirtualRegister(&OPU::SReg_32RegClass);
    BuildMI(MBB, I, DL, TII.get(OPU::COPY), TmpReg)
      .addReg(SrcReg);
    BuildMI(MBB, I, DL, TII.get(OPU::COPY), OPU::SCC)
      .addReg(TmpReg);
  ...
  if (SrcBank->getID() == OPU::VCCRegBankID && DstSize <= 32) {
    if (SrcTy != S1) // Invalid
      return false;

    MachineInstr *ExtI =
      BuildMI(MBB, I, DL, TII.get(OPU::V_CNDMASK_B32_e64), DstReg)
  ...
  if (I.getOpcode() == OPU::G_ANYEXT)
    return selectCOPY(I);

  if (SrcBank->getID() == OPU::VPRRegBankID && DstSize <= 32) {
    ...
    if (!Signed && shouldUseAndMask(SrcSize, Mask)) {
      MachineInstr *ExtI =
      BuildMI(MBB, I, DL, TII.get(OPU::V_AND_B32_e32), DstReg)
        .addImm(Mask)
        .addReg(SrcReg);
      I.eraseFromParent();
      return constrainSelectedInstRegOperands(*ExtI, TII, TRI, RBI);
    ...
    const unsigned BFE = Signed ? OPU::V_BFE_I32 : OPU::V_BFE_U32;
    MachineInstr *ExtI =
  ...
  if (SrcBank->getID() == OPU::SPRRegBankID && DstSize <= 64) {
    if (!RBI.constrainGenericRegister(SrcReg, OPU::SReg_32RegClass, MRI))
      return false;

    if (Signed && DstSize == 32 && (SrcSize == 8 || SrcSize == 16)) {
      const unsigned SextOpc = SrcSize == 8 ?
        OPU::S_SEXT_I32_I8 : OPU::S_SEXT_I32_I16;
      BuildMI(MBB, I, DL, TII.get(SextOpc), DstReg)
        .addReg(SrcReg);
      I.eraseFromParent();
      return RBI.constrainGenericRegister(DstReg, OPU::SReg_32RegClass, MRI);
    }

    const unsigned BFE64 = Signed ? OPU::S_BFE_I64 : OPU::S_BFE_U64;
    const unsigned BFE32 = Signed ? OPU::S_BFE_I32 : OPU::S_BFE_U32;
    ...
    if (DstSize > 32 && SrcSize <= 32) {
      ...
      Register ExtReg = MRI.createVirtualRegister(&OPU::SReg_64RegClass);
      Register UndefReg = MRI.createVirtualRegister(&OPU::SReg_32RegClass);
      BuildMI(MBB, I, DL, TII.get(OPU::IMPLICIT_DEF), UndefReg);
      BuildMI(MBB, I, DL, TII.get(OPU::REG_SEQUENCE), ExtReg)
        .addReg(SrcReg)
        .addImm(OPU::sub0)
        .addReg(UndefReg)
        .addImm(OPU::sub1);
      ...
      I.eraseFromParent();
      return RBI.constrainGenericRegister(DstReg, OPU::SReg_64RegClass, MRI);
    }

    unsigned Mask;
    if (!Signed && shouldUseAndMask(SrcSize, Mask)) {
      BuildMI(MBB, I, DL, TII.get(OPU::S_AND_B32), DstReg)
        .addReg(SrcReg)
        .addImm(Mask);
    } else {
...
bool OPUInstructionSelector::selectG_CONSTANT(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  ...
  if (RB) {
    IsSgpr = RB->getID() == OPU::SPRRegBankID;
  ...
  unsigned Opcode = IsSgpr ? OPU::S_MOV_B32 : OPU::V_MOV_B32_e32;
  if (Size == 32) {
  ...
  DebugLoc DL = I.getDebugLoc();
  const TargetRegisterClass *RC = IsSgpr ? &OPU::SReg_32RegClass :
                                           &OPU::VPR_32RegClass;
...
void OPUInstructionSelector::getAddrModeInfo(const MachineInstr &Load,
  ...
  for (unsigned i = 1, e = 3; i < e; ++i) {
    ...
    if (OpBank->getID() == OPU::SPRRegBankID)
...
bool OPUInstructionSelector::isInstrUniform(const MachineInstr &MI) const {
...
bool OPUInstructionSelector::hasVgprParts(ArrayRef<GEPInfo> AddrInfo) const {
...
void OPUInstructionSelector::initM0(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  ...
  if ((AS == AMDGPUAS::LOCAL_ADDRESS || AS == AMDGPUAS::REGION_ADDRESS) &&
    ...
    BuildMI(*BB, &I, I.getDebugLoc(), TII.get(OPU::S_MOV_B32), OPU::M0)
      .addImm(-1);
  }
}

bool OPUInstructionSelector::selectG_LOAD_ATOMICRMW(MachineInstr &I) const {
  initM0(I);
  return selectImpl(I, *CoverageInfo);
}

bool OPUInstructionSelector::selectG_BRCOND(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  ...
  if (isSCC(CondReg, MRI)) {
    CondPhysReg = OPU::SCC;
    BrOpcode = OPU::S_CBRANCH_SCC1;
    ConstrainRC = &OPU::SReg_32RegClass;
  ...
  BuildMI(*BB, &I, DL, TII.get(OPU::COPY), CondPhysReg)
    .addReg(CondReg);
...
bool OPUInstructionSelector::selectG_FRAME_INDEX(MachineInstr &I) const {
  MachineBasicBlock *BB = I.getParent();
  MachineFunction *MF = BB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  ...
  Register DstReg = I.getOperand(0).getReg();
  const RegisterBank *DstRB = RBI.getRegBank(DstReg, MRI, TRI);
  const bool IsVGPR = DstRB->getID() == OPU::VPRRegBankID;
  I.setDesc(TII.get(IsVGPR ? OPU::V_MOV_B32_e32 : OPU::S_MOV_B32));
  if (IsVGPR)
    I.addOperand(*MF, MachineOperand::CreateReg(OPU::TMSK, false, true));

  return RBI.constrainGenericRegister(
    DstReg, IsVGPR ? OPU::VPR_32RegClass : OPU::SReg_32RegClass, MRI);
}
...
bool OPUInstructionSelector::select(MachineInstr &I) {
  ...
  case TargetOpcode::G_FENCE:
    ...
    I.setDesc(TII.get(OPU::ATOMIC_FENCE));
    return true;
  }
  return false;
}
...
OPUInstructionSelector::selectVCSRC(MachineOperand &Root) const {
  return {{
...
OPUInstructionSelector::selectVOP3ModsImpl(
  Register Src, const MachineRegisterInfo &MRI) const {
  unsigned Mods = 0;
  MachineInstr *MI = MRI.getVRegDef(Src);

  if (MI && MI->getOpcode() == OPU::G_FNEG) {
    Src = MI->getOperand(1).getReg();
    Mods |= OPUSrcMods::NEG;
    MI = MRI.getVRegDef(Src);
  }

  if (MI && MI->getOpcode() == OPU::G_FABS) {
    Src = MI->getOperand(1).getReg();
    Mods |= OPUSrcMods::ABS;
  }
...
InstructionSelector::ComplexRendererFns
OPUInstructionSelector::selectVSRC0(MachineOperand &Root) const {
  return {{
      [=](MachineInstrBuilder &MIB) { MIB.add(Root); }
  }};
}
...
InstructionSelector::ComplexRendererFns
OPUInstructionSelector::selectVOP3Mods0(MachineOperand &Root) const {
...
InstructionSelector::ComplexRendererFns
OPUInstructionSelector::selectVOP3OMods(MachineOperand &Root) const {
  return {{
      [=](MachineInstrBuilder &MIB) { MIB.add(Root); },
      [=](MachineInstrBuilder &MIB) { MIB.addImm(0); }, // clamp
      [=](MachineInstrBuilder &MIB) { MIB.addImm(0); }  // omod
...
InstructionSelector::ComplexRendererFns
OPUInstructionSelector::selectVOP3Mods(MachineOperand &Root) const {
  MachineRegisterInfo &MRI
    = Root.getParent()->getParent()->getParent()->getRegInfo();
...
InstructionSelector::ComplexRendererFns
OPUInstructionSelector::selectSmrdImm(MachineOperand &Root) const {
  MachineRegisterInfo &MRI =
      Root.getParent()->getParent()->getParent()->getRegInfo();
  ...
  if (!OPU::isLegalSMRDImmOffset(STI, GEPInfo.Imm))
    return None;

  unsigned PtrReg = GEPInfo.SgprParts[0];
  int64_t EncodedImm = OPU::getSMRDEncodedOffset(STI, GEPInfo.Imm);
  return {{
...
InstructionSelector::ComplexRendererFns
OPUInstructionSelector::selectSmrdImm32(MachineOperand &Root) const {
  MachineRegisterInfo &MRI =
      Root.getParent()->getParent()->getParent()->getRegInfo();
...
InstructionSelector::ComplexRendererFns
OPUInstructionSelector::selectSmrdSgpr(MachineOperand &Root) const {
  ...
  unsigned PtrReg = GEPInfo.SgprParts[0];
  Register OffsetReg = MRI.createVirtualRegister(&OPU::SReg_32RegClass);
  BuildMI(*MBB, MI, MI->getDebugLoc(), TII.get(OPU::S_MOV_B32), OffsetReg)
          .addImm(GEPInfo.Imm);
  return {{
    [=](MachineInstrBuilder &MIB) { MIB.addReg(PtrReg); },
...
InstructionSelector::ComplexRendererFns
OPUInstructionSelector::selectFlatOffsetImpl(MachineOperand &Root) const {
  MachineInstr *MI = Root.getParent();
  MachineBasicBlock *MBB = MI->getParent();
  MachineRegisterInfo &MRI = MBB->getParent()->getRegInfo();
  ...
  if (!OpDef || OpDef->getOpcode() != OPU::G_GEP)
    return Default;
...
InstructionSelector::ComplexRendererFns
OPUInstructionSelector::selectFlatOffset(MachineOperand &Root) const {
  return selectFlatOffsetImpl<false>(Root);
}

InstructionSelector::ComplexRendererFns
OPUInstructionSelector::selectFlatOffsetSigned(MachineOperand &Root) const {
...
InstructionSelector::ComplexRendererFns
OPUInstructionSelector::selectMUBUFScratchOffen(MachineOperand &Root) const {
  MachineInstr *MI = Root.getParent();
  MachineBasicBlock *MBB = MI->getParent();
  MachineFunction *MF = MBB->getParent();
  MachineRegisterInfo &MRI = MF->getRegInfo();
  ...
  if (mi_match(Root.getReg(), MRI, m_ICst(Offset))) {
    Register HighBits = MRI.createVirtualRegister(&OPU::VPR_32RegClass);

    // TODO: Should this be inside the render function? The iterator seems to
    // move.
    BuildMI(*MBB, MI, MI->getDebugLoc(), TII.get(OPU::V_MOV_B32_e32),
            HighBits)
      .addImm(Offset & ~4095);
  ...
  if (const MachineInstr *RootDef = MRI.getVRegDef(Root.getReg())) {
    if (isBaseWithConstantOffset(Root, MRI)) {
      ...
      if (LHSDef && RHSDef) {
        ...
        if (OPUInstrInfo::isLegalMUBUFImmOffset(PossibleOffset) &&
            (!STI.privateMemoryResourceIsRangeChecked() ||
             signBitIsZero(LHS, MRI))) {
          if (LHSDef->getOpcode() == OPU::G_FRAME_INDEX)
    ...
    } else if (RootDef->getOpcode() == OPU::G_FRAME_INDEX) {
...
bool OPUInstructionSelector::isDSOffsetLegal(const MachineRegisterInfo &MRI,
...
InstructionSelector::ComplexRendererFns
OPUInstructionSelector::selectMUBUFScratchOffset(
  ...
  int64_t Offset = 0;
  if (!mi_match(Root.getReg(), MRI, m_ICst(Offset)) ||
      !OPUInstrInfo::isLegalMUBUFImmOffset(Offset))
    return {};
...
InstructionSelector::ComplexRendererFns
OPUInstructionSelector::selectDS1Addr1Offset(MachineOperand &Root) const {
  MachineInstr *MI = Root.getParent();
  MachineBasicBlock *MBB = MI->getParent();
  MachineRegisterInfo &MRI = MBB->getParent()->getRegInfo();
  ...
  } else if (RootDef->getOpcode() == OPU::G_SUB) {
