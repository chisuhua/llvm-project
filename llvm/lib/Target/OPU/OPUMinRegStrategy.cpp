...
///
/// \file
/// This file defines and imlements the class OPUMinRegScheduler, which
...
void OPUMinRegScheduler::initNumPreds(const decltype(ScheduleDAG::SUnits) &SUnits) {
  NumPreds.resize(SUnits.size());
  for (unsigned I = 0; I < SUnits.size(); ++I)
    NumPreds[I] = SUnits[I].NumPredsLeft;
}

int OPUMinRegScheduler::getReadySuccessors(const SUnit *SU) const {
...
int OPUMinRegScheduler::getNotReadySuccessors(const SUnit *SU) const {
  return SU->Succs.size() - getReadySuccessors(SU);
}

template <typename Calc>
unsigned OPUMinRegScheduler::findMax(unsigned Num, Calc C) {
...
OPUMinRegScheduler::Candidate* OPUMinRegScheduler::pickCandidate() {
  do {
...
void OPUMinRegScheduler::bumpPredsPriority(const SUnit *SchedSU, int Priority) {
...
void OPUMinRegScheduler::releaseSuccessors(const SUnit* SU, int Priority) {
...
OPUMinRegScheduler::schedule(ArrayRef<const SUnit*> TopRoots,
                             const ScheduleDAG &DAG) {
