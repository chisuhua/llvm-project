...
std::vector<const SUnit *> makeOPUILPScheduler(ArrayRef<const SUnit *> BotRoots,
    const ScheduleDAG &DAG);
}
...
static void printLivenessInfo(raw_ostream &OS,
  ...
  OS << "LIn RP: ";
  OPU::getRegPressure(MRI, LiveIns).print(OS);

  const auto BottomMI = End == BB->end() ? std::prev(End) : End;
  const auto LiveOuts = getLiveRegsAfter(*BottomMI, *LIS);
  OS << "LOt RP: ";
  OPU::getRegPressure(MRI, LiveOuts).print(OS);
}

LLVM_DUMP_METHOD
void OPUIterativeScheduler::printRegions(raw_ostream &OS) const {
  const auto &ST = MF.getSubtarget<OPUSubtarget>();
...
LLVM_DUMP_METHOD
void OPUIterativeScheduler::printSchedResult(raw_ostream &OS,
                                            const Region *R,
...
LLVM_DUMP_METHOD
void OPUIterativeScheduler::printSchedRP(raw_ostream &OS,
...
// DAG builder helper
class OPUIterativeScheduler::BuildDAG {
  OPUIterativeScheduler &Sch;
  SmallVector<SUnit *, 8> TopRoots;

  SmallVector<SUnit*, 8> BotRoots;

public:
  BuildDAG(const Region &R, OPUIterativeScheduler &_Sch)
    : Sch(_Sch) {
...
OPUIterativeScheduler::OPUIterativeScheduler(MachineSchedContext *C,
                                           StrategyKind S, int Margin, int Inc)
...
OPURegPressure OPUIterativeScheduler::getRegionPressure(MachineBasicBlock::iterator Begin,
...
template <typename Range> OPURegPressure
OPUIterativeScheduler::getSchedulePressure(const Region &R,
                                          Range &&Schedule) const {
  auto const BBEnd = R.Begin->getParent()->end();
  OPUUpwardRPTracker RPTracker(*LIS);
  if (R.End != BBEnd) {
...
void OPUIterativeScheduler::enterRegion(MachineBasicBlock *BB, // overriden
                                        MachineBasicBlock::iterator Begin,
                                        MachineBasicBlock::iterator End,
...
void OPUIterativeScheduler::schedule() { // overriden
  // do nothing
  LLVM_DEBUG(printLivenessInfo(dbgs(), RegionBegin, RegionEnd, LIS);
             if (!Regions.empty() && Regions.back()->Begin == RegionBegin) {
               ...
               Regions.back()->MaxPressure.print(
                   dbgs(), &MF.getSubtarget<OPUSubtarget>());
             } dbgs()
             << '\n';);
}

void OPUIterativeScheduler::finalizeSchedule() { // overriden
  if (Regions.empty())
    return;
  switch (Strategy) {
...
OPUIterativeScheduler::detachSchedule(ScheduleRef Schedule) const {
...
void OPUIterativeScheduler::setBestSchedule(Region &R, ScheduleRef Schedule,
                                           const OPURegPressure &MaxRP) {
  R.BestSchedule.reset(new TentativeSchedule{detachSchedule(Schedule), MaxRP});
}

void OPUIterativeScheduler::scheduleBest(Region &R) {
...
void OPUIterativeScheduler::scheduleRegion(Region &R, Range &&Schedule,
                                          const OPURegPressure &MaxRP) {
...
void OPUIterativeScheduler::sortRegionsByPressure(unsigned TargetOcc) {
  const auto &ST = MF.getSubtarget<OPUSubtarget>();
...
unsigned OPUIterativeScheduler::tryMaximizeOccupancy(unsigned TargetOcc) {
...
void OPUIterativeScheduler::scheduleLegacyMaxOccupancy(
  ...
  LLVM_DEBUG(dbgs() << "Scheduling using default scheduler, "
                       "target occupancy = "
                    << TgtOcc << '\n');
  OPUMaxOccupancySchedStrategy LStrgy(Context, ErrorMargin, RegPresureInc, false);
...
void OPUIterativeScheduler::scheduleMinReg(bool force) {
  const auto &ST = MF.getSubtarget<OPUSubtarget>();
...
void OPUIterativeScheduler::scheduleILP(bool TryMaximizeOccupancy) {
  const auto &ST = MF.getSubtarget<OPUSubtarget>();
  OPUMachineFunctionInfo *MFI = MF.getInfo<OPUMachineFunctionInfo>();
  auto TgtOcc = MFI->getMinAllowedOccupancy();
  ...
  for (auto R : Regions) {
    BuildDAG DAG(*R, *this);
    const auto ILPSchedule = makeOPUILPScheduler(DAG.getBottomRoots(), *this);
