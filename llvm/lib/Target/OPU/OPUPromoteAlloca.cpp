//===-- OPUPromoteAlloca.cpp - Promote Allocas -------------------------===//
//
...
class OPUPromoteAlloca : public FunctionPass {
private:
...
public:
  ...
  OPUPromoteAlloca() : FunctionPass(ID) {}

  bool doInitialization(Module &M) override;
  bool runOnFunction(Function &F) override;

  StringRef getPassName() const override { return "OPU Promote Alloca"; }
...
char OPUPromoteAlloca::ID = 0;

INITIALIZE_PASS(OPUPromoteAlloca, DEBUG_TYPE,
                "OPU promote alloca to vector or LDS", false, false)

char &llvm::OPUPromoteAllocaID = OPUPromoteAlloca::ID;

bool OPUPromoteAlloca::doInitialization(Module &M) {
  Mod = &M;
...
bool OPUPromoteAlloca::runOnFunction(Function &F) {
  if (skipFunction(F))
    return false;
  ...
  IsOPU = TT.getArch() == Triple::ppu;
  IsPPS = TT.getOS() == Triple::PPS;

  const OPUSubtarget &ST = OPUSubtarget::get(*TM, F);
...
OPUPromoteAlloca::getLocalSizeYZ(IRBuilder<> &Builder) {
...
Value *OPUPromoteAlloca::getWorkitemID(IRBuilder<> &Builder, unsigned N) {
  const OPUSubtarget &ST =
      OPUSubtarget::get(*TM, *Builder.GetInsertBlock()->getParent());
...
bool OPUPromoteAlloca::binaryOpIsDerivedFromSameAlloca(Value *BaseAlloca,
                                                            Value *Val,
                                                            Instruction *Inst,
                                                            int OpIdx0,
                                                            int OpIdx1) const {
...
bool OPUPromoteAlloca::collectUsesWithPtrTypes(
...
bool OPUPromoteAlloca::hasSufficientLocalMem(const Function &F) {
...
bool OPUPromoteAlloca::handleAlloca(AllocaInst &I, bool SufficientLDS) {
  ...
  const OPUSubtarget &ST = OPUSubtarget::get(*TM, ContainingFunction);
...
FunctionPass *llvm::createOPUPromoteAlloca() {
  return new OPUPromoteAlloca();
}
