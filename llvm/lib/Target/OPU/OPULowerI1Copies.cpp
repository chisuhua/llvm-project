//===-- OPULowerI1Copies.cpp - Lower I1 Copies -----------------------------===//
//
...
#define DEBUG_TYPE "opu-i1-copies"
...
class OPULowerI1Copies : public MachineFunctionPass {
public:
  static char ID;
...
public:
  OPULowerI1Copies() : MachineFunctionPass(ID) {
    initializeOPULowerI1CopiesPass(*PassRegistry::getPassRegistry());
  }

  bool runOnMachineFunction(MachineFunction &MF) override;

  StringRef getPassName() const override { return "OPU Lower i1 Copies"; }
...
private:
  void lowerCopiesFromVcmpToI1();
  void lowerCopiesFromI1();
  void lowerPhis();
  void lowerCopiesToI1();
  void setAllocationHint();
  ...
  bool isVreg1(unsigned Reg) const {
    return TargetRegisterInfo::isVirtualRegister(Reg) &&
           MRI->getRegClass(Reg) == &OPU::VReg_1RegClass;
  }
  ...
  void analyze(MachineBasicBlock &DefBlock,
    ...
    for (MachineBasicBlock *MBB : IncomingBlocks) {
      ...
      for (MachineInstr &MI : MBB->terminators()) {
        if (MI.getOpcode() == OPU::IF ||
            MI.getOpcode() == OPU::ELSE ||
            MI.getOpcode() == OPU::LOOP) {
          Divergent = true;
          break;
        }
      }
    ...
    // and a patch for special case:
    // bb.0:
    //  %39: cgpr_32 = s_mov_b32_imm - 1
    //  %3: cgpr_32 = IF ... %bb.11
    //  s_cbr %bb.1
    // bb.1:
    //   %67:cgpr_32 = s_mov_b32_imm 0
    //   s_cbr_scc0 %bb.11, implicit %scc
    //   s_cbr %bb.2
    // bb2 -> bb3 --> bb4 --> bb5 --> bb8
    //                  \---> bb6
    // bb.8:
    ...
    DenseMap<MachineBasicBlock *, bool> Visited;
    while (!Stack.empty()) {
      ...
      if (MBB == &DefBlock || Visited.count(MBB)) continue;
...
INITIALIZE_PASS_BEGIN(OPULowerI1Copies, DEBUG_TYPE, "OPU Lower i1 Copies", false,
                      false)
INITIALIZE_PASS_DEPENDENCY(MachineDominatorTree)
INITIALIZE_PASS_DEPENDENCY(MachinePostDominatorTree)
INITIALIZE_PASS_END(OPULowerI1Copies, DEBUG_TYPE, "OPU Lower i1 Copies", false,
                    false)

char OPULowerI1Copies::ID = 0;

char &llvm::OPULowerI1CopiesID = OPULowerI1Copies::ID;

FunctionPass *llvm::createOPULowerI1CopiesPass() {
  return new OPULowerI1Copies();
}

static unsigned createLaneMaskReg(MachineFunction &MF) {
  const OPUSubtarget &ST = MF.getSubtarget<OPUSubtarget>();
  MachineRegisterInfo &MRI = MF.getRegInfo();
  return MRI.createVirtualRegister(&OPU::SReg_32RegClass);
}
...
static unsigned insertUndefLaneMask(MachineBasicBlock &MBB) {
  ...
  BuildMI(MBB, MBB.getFirstTerminator(), {}, TII->get(OPU::IMPLICIT_DEF),
          UndefReg);
  return UndefReg;
}
...
bool OPULowerI1Copies::runOnMachineFunction(MachineFunction &TheMF) {
  MF = &TheMF;
  MRI = &MF->getRegInfo();
  ...
  OrOp = OPU::S_OR_B32;
  XorOp = OPU::S_XOR_B32;
  AndN2Op = OPU::S_LOP2_B32;
  OrN2Op = OPU::S_LOP2_B32;

  lowerCopiesFromVcmpToI1();
  lowerCopiesFromI1();
  lowerPhis();
  lowerCopiesToI1();

  for (unsigned Reg : ConstrainRegs)
    MRI->constrainRegClass(Reg, &OPU::CGPR_32_TMSK_SCCRegClass);
  ConstrainRegs.clear();
...
#ifndef NDEBUG
static bool isVregCompatibleReg(const OPURegisterInfo &TRI,
...
void OPULowerI1Copies::lowerCopiesFromVcmpToI1() {
  for (MachineBasicBlock &MBB : *MF) {
    for (MachineInstr &MI : MBB) {
      ...
      MRI->setRegClass(DstReg, &OPU::CGPR_32RegClass);
    }
  }
}

void OPULowerI1Copies::lowerCopiesFromI1() {
  ...
  for (MachineBasicBlock &MBB : *MF) {
    for (MachineInstr &MI : MBB) {
      ...
      BuildMI(MBB, MI, DL, TII->get(OPU::V_CSEL_B32_IMM), DstReg)
          .addReg(SrcReg)
          .addReg(OPU::IMPCONS_NEG1);
...
void OPULowerI1Copies::lowerPhis() {
  ...
  for (MachineInstr *MI : Vreg1Phis) {
      ...
      for (unsigned i = 1; i < MI.getNumOperands(); i += 2) {
        ...
        if (IncomingDef->getOpcode() == OPU::COPY) {
        ...
        } else if (IncomingDef->getOpcode() == OPU::IMPLICIT_DEF) {
          continue;
        } else {
          assert(isLaneMaskReg(IncomingReg) || isVreg1(IncomingReg));
...
void OPULowerI1Copies::lowerCopiesToI1() {
  MachineSSAUpdater SSAUpdater(*MF);
  LoopFinder LF(*DT, *PDT);
  SmallVector<MachineInstr *, 4> DeadCopies;
  ...
  for (MachineBasicBlock &MBB : *MF) {
    ...
    for (MachineInstr &MI : MBB) {
      if (MI.getOpcode() != OPU::IMPLICIT_DEF &&
          MI.getOpcode() != OPU::COPY)
        continue;
      ...
      if (!TargetRegisterInfo::isVirtualRegister(SrcReg) ||
        ...
        BuildMI(MBB, MI, DL, TII->get(OPU::V_CMP_NE_U32_IMM), TmpReg)
            .addReg(SrcReg)
            .addImm(0);
...
void OPULowerI1Copies::setAllocationHint() {
  for (MachineBasicBlock &MBB : *MF) {
    for (MachineInstr &MI : MBB) {
      if (TII->isVOPC(MI)) {
        MachineOperand *Dst = TII->getNamedOperand(MI, OPU::OpName::dst);
        if (Dst && Register::isVirtualRegister(Dst->getReg())) {
          if (Register::isVirtualRegister(Dst->getReg()))
            MRI->setRegAllocationHint(Dst->getReg(), 0, OPU::VCC);
        }
      } else if (TII->isSOPC(MI)){
        MachineOperand *Dst = TII->getNamedOperand(MI, OPU::OpName::dst);
        if (Dst && Register::isVirtualRegister(Dst->getReg())) {
          if (Register::isVirtualRegister(Dst->getReg()))
            MRI->setRegAllocationHint(Dst->getReg(), 0, OPU::SCC);
        }
      }
    }
  }
...
bool OPULowerI1Copies::isConstantLaneMask(unsigned Reg, bool &Val) const {
...
MachineBasicBlock::iterator
OPULowerI1Copies::getSaluInsertionAtEnd(MachineBasicBlock &MBB) const {
...
void OPULowerI1Copies::buildMergeLaneMasks(MachineBasicBlock &MBB,
  ...
  if (PrevConstant && CurConstant) {
    if (PrevVal == CurVal) {
      BuildMI(MBB, I, DL, TII->get(OPU::COPY), DstReg).addReg(CurReg);
    } else if (CurVal) {
      BuildMI(MBB, I, DL, TII->get(OPU::COPY), DstReg).addReg(ExecReg);
    } else {
  ...
  if (PrevConstant && !PrevVal) {
    BuildMI(MBB, I, DL, TII->get(OPU::COPY), DstReg)
        .addReg(CurMaskedReg);
