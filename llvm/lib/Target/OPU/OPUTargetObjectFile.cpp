//===-- OPUTargetObjectFile.cpp - OPU Object Info -----------------===//
//
...
void OPUELFTargetObjectFile::Initialize(MCContext &Ctx,
...
bool OPUELFTargetObjectFile::isInSmallSection(uint64_t Size) const {
...
// section.
bool OPUELFTargetObjectFile::isGlobalInSmallSection(
...
MCSection *OPUELFTargetObjectFile::SelectSectionForGlobal(
...
MCSection *OPUELFTargetObjectFile::getExplicitSectionGlobal(
    const GlobalObject *GO, SectionKind Kind, const TargetMachine &TM) const {
  // Handle Small Section classification here.

  // Otherwise, we work the same as ELF.
  return TargetLoweringObjectFileELF::getExplicitSectionForGlobal(GO, Kind, TM);
}

void OPUELFTargetObjectFile::getModuleMetadata(Module &M) {
...
bool OPUELFTargetObjectFile::isConstantInSmallSection(
    const DataLayout &DL, const Constant *CN) const {
  return isInSmallSection(DL.getTypeAllocSize(CN->getType()));
}

MCSection *OPUELFTargetObjectFile::getSectionForConstant(
