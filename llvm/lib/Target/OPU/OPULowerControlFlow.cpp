//===-- OPULowerControlFlow.cpp - Use predicates for control flow ----------===//
//
...
#define DEBUG_TYPE "opu-lower-control-flow"

namespace {

class OPULowerControlFlow : public MachineFunctionPass {
private:
...
public:
  ...
  OPULowerControlFlow() : MachineFunctionPass(ID) {}
...
char OPULowerControlFlow::ID = 0;

INITIALIZE_PASS(OPULowerControlFlow, DEBUG_TYPE,
               "OPU lower control flow", false, false)

static void setImpSCCDefDead(MachineInstr &MI, bool IsDead) {
  // MachineOperand &ImpDefSCC = MI.getOperand(3);
  //assert(ImpDefSCC.getReg() == OPU::SCC && ImpDefSCC.isDef());

  //ImpDefSCC.setIsDead(IsDead);
}

char &llvm::OPULowerControlFlowID = OPULowerControlFlow::ID;
...
static bool isSimpleIf(const MachineInstr &MI, const MachineRegisterInfo *MRI,
  ...
  if (U == MRI->use_instr_nodbg_end() ||
      ...
      U->getOpcode() != OPU::END_CF)
    return false;

  // Check for OPU_KILL_*_TERMINATOR on path from if to endif.
...
Register OPULowerControlFlow::getSaveExec(MachineInstr *MI) {
  MachineBasicBlock *MBB = MI->getParent();
  MachineOperand &SaveExec = MI->getOperand(0);
  assert(SaveExec.getSubReg() == OPU::NoSubRegister);

  Register SaveExecReg = SaveExec.getReg();
  unsigned FalseTermOpc = OPU::S_MOV_B32_term;
...
void OPULowerControlFlow::emitIf(MachineInstr &MI) {
  MachineBasicBlock &MBB = *MI.getParent();
  ...
  // assert(SaveExec.getSubReg() == OPU::NoSubRegister &&
  //       Cond.getSubReg() == OPU::NoSubRegister);
  assert(Cond.getSubReg() == OPU::NoSubRegister);
  ...
  // If there is only one use of save exec register and that use is OPU_END_CF,
  // we can optimize OPU_IF by returning the full saved exec mask instead of
  // just cleared bits.
  ...
  MachineInstr *CopyExec =
    BuildMI(MBB, I, DL, TII->get(OPU::COPY), CopyReg)
    .addReg(Exec)
  ...
  MachineInstr *NewBr = BuildMI(MBB, I, DL, TII->get(OPU::MASK_BRANCH))
                            .add(MI.getOperand(2));
  ...
  LIS->removeAllRegUnitsForPhysReg(OPU::EXEC);
  MI.eraseFromParent();
...
void OPULowerControlFlow::emitElse(MachineInstr &MI) {
  MachineBasicBlock &MBB = *MI.getParent();
  const DebugLoc &DL = MI.getDebugLoc();
  ...
  MachineInstr *CopyExec =
    BuildMI(MBB, Start, DL, TII->get(OPU::COPY), CopyReg)
      .add(MI.getOperand(1)); // Saved EXEC

  // This must be inserted before phis and any spill code inserted before the
  // else.
  Register SaveReg = DstReg;

  MachineInstr *OrSaveExec =
    BuildMI(MBB, Start, DL, TII->get(LopTmskOpc), SaveReg)
    .addReg(CopyReg);
    .addImm(0xe)
  ...
  MachineInstr *Branch =
    BuildMI(MBB, ElsePt, DL, TII->get(OPU::MASK_BRANCH))
    .addMBB(DestBB);
  ...
  LIS->removeAllRegUnitsForPhysReg(OPU::TMSK);
}

void OPULowerControlFlow::emitIfBreak(MachineInstr &MI) {
  MachineBasicBlock &MBB = *MI.getParent();
  const DebugLoc &DL = MI.getDebugLoc();
...
void OPULowerControlFlow::emitLoop(MachineInstr &MI) {
  MachineBasicBlock &MBB = *MI.getParent();
  const DebugLoc &DL = MI.getDebugLoc();

  // ta = 0b1100
  // tb = 0b1010
  // a & ~b = 0b1100 & (~0b1010) = 0x4
  MachineInstr *AndN2 =
      BuildMI(MBB, &MI, DL, TII->get(Lop2TermOpc), Exec)
          .addReg(Exec)
          .add(MI.getOperand(0));
          .addImm(0x4)

  MachineInstr *Branch =
      BuildMI(MBB, &MI, DL, TII->get(OPU::S_CBRANCH_TMSKNZ))
          .add(MI.getOperand(1));
...
void OPULowerControlFlow::emitEndCf(MachineInstr &MI) {
  MachineBasicBlock &MBB = *MI.getParent();
...
void OPULowerControlFlow::findMaskOperands(MachineInstr &MI, unsigned OpNo,
       SmallVectorImpl<MachineOperand> &Src) const {
  ...
  for (auto I = Def->getIterator(); I != MI.getIterator(); ++I)
    if (I->modifiesRegister(OPU::TMSK, TRI) &&
...
void OPULowerControlFlow::combineMasks(MachineInstr &MI) {
...
bool OPULowerControlFlow::runOnMachineFunction(MachineFunction &MF) {
  ...
  MRI = &MF.getRegInfo();
  BoolRC = &OPU::CGPR_32RegClass; // TRI->getBoolRC();

  MovOpc = OPU::S_MOV_B32;
  AndOpc = OPU::S_AND_B32;
  OrOpc = OPU::S_OR_B32;
  XorOpc = OPU::S_XOR_B32;
  LopTmskOpc = OPU::S_LOP_TMSK;
  MovTermOpc = OPU::S_MOV_B32_term;
  AndTermOpc = OPU::S_AND_B32_term;
  XorTermrOpc = OPU::S_XOR_B32_term;
  Lop2TermrOpc = OPU::S_LOP2_B32_term;
  Exec = OPU::TMSK;
