//===- OPUInstrInfo.cpp - OPU Instruction Information  ----------------------===//
//
...
//
/// \file
/// OPU Implementation of TargetInstrInfo.
//
//===----------------------------------------------------------------------===//
...
void OPUInstrInfo::anchor() {}




bool OPUInstrInfo::areLoadsFromSameBasePtr(SDNode *Load0, SDNode *Load1,
                                          int64_t &Offset0,
  ...
  if (isDS(Opc0) && isDS(Opc1)) {
    ...
    // st64 versions).
    int Offset0Idx = OPU::getNamedOperandIdx(Opc0, OPU::OpName::offset);
    int Offset1Idx = OPU::getNamedOperandIdx(Opc1, OPU::OpName::offset);
  ...
  if (isSMRD(Opc0) && isSMRD(Opc1)) {
    ...
    if (OPU::getNamedOperandIdx(Opc0, OPU::OpName::sbase) == -1 ||
        OPU::getNamedOperandIdx(Opc1, OPU::OpName::sbase) == -1)
      return false;
  ...
  if ((isMUBUF(Opc0) || isMTBUF(Opc0)) && (isMUBUF(Opc1) || isMTBUF(Opc1))) {
    ...
    if (!nodesHaveSameOperandValue(Load0, Load1, OPU::OpName::soffset) ||
        !nodesHaveSameOperandValue(Load0, Load1, OPU::OpName::vaddr) ||
        !nodesHaveSameOperandValue(Load0, Load1, OPU::OpName::srsrc))
      return false;

    int OffIdx0 = OPU::getNamedOperandIdx(Opc0, OPU::OpName::offset);
    int OffIdx1 = OPU::getNamedOperandIdx(Opc1, OPU::OpName::offset);
...
static bool isStride64(unsigned Opc) {
  switch (Opc) {
  case OPU::DS_READ2ST64_B32:
  case OPU::DS_READ2ST64_B64:
  case OPU::DS_WRITE2ST64_B32:
  case OPU::DS_WRITE2ST64_B64:
    return true;
  default:
    return false;
  }
...
bool OPUInstrInfo::getMemOperandWithOffset(const MachineInstr &LdSt,
  ...
  if (isDS(LdSt)) {
    const MachineOperand *OffsetImm =
        getNamedOperand(LdSt, OPU::OpName::offset);
    if (OffsetImm) {
      // Normal, single offset LDS instruction.
      BaseOp = getNamedOperand(LdSt, OPU::OpName::addr);
    ...
    const MachineOperand *Offset0Imm =
        getNamedOperand(LdSt, OPU::OpName::offset0);
    const MachineOperand *Offset1Imm =
        getNamedOperand(LdSt, OPU::OpName::offset1);
    ...
    if (Offset1 > Offset0 && Offset1 - Offset0 == 1) {
      ...
      if (LdSt.mayLoad())
        ...
        int Data0Idx = OPU::getNamedOperandIdx(Opc, OPU::OpName::data0);
  ...
  if (isMUBUF(LdSt) || isMTBUF(LdSt)) {
    const MachineOperand *SOffset = getNamedOperand(LdSt, OPU::OpName::soffset);
    if (SOffset && SOffset->isReg())
      return false;

    const MachineOperand *AddrReg = getNamedOperand(LdSt, OPU::OpName::vaddr);
    if (!AddrReg)
      return false;
  ...
  if (isSMRD(LdSt)) {
    ...
    const MachineOperand *SBaseReg = getNamedOperand(LdSt, OPU::OpName::sbase);
    BaseOp = SBaseReg;
    Offset = OffsetImm->getImm();
    assert(BaseOp->isReg() && "getMemOperandWithOffset only supports base "
                              "operands of type register.");
  ...
  if (isFLAT(LdSt)) {
    const MachineOperand *VAddr = getNamedOperand(LdSt, OPU::OpName::vaddr);
    if (VAddr) {
      // Can't analyze 2 offsets.
      if (getNamedOperand(LdSt, OPU::OpName::saddr))
        return false;

      BaseOp = VAddr;
    } else {
      // scratch instructions have either vaddr or saddr.
      BaseOp = getNamedOperand(LdSt, OPU::OpName::saddr);
    }

    Offset = getNamedOperand(LdSt, OPU::OpName::offset)->getImm();
    assert(BaseOp->isReg() && "getMemOperandWithOffset only supports base "
                              "operands of type register.");
    return true;
  }
...
bool OPUInstrInfo::shouldClusterMemOps(const MachineOperand &BaseOp1,
                                      const MachineOperand &BaseOp2,
  ...
  if ((isMUBUF(FirstLdSt) && isMUBUF(SecondLdSt)) ||
    ...
    FirstDst = getNamedOperand(FirstLdSt, OPU::OpName::vdata);
    if (!FirstDst)
      FirstDst = getNamedOperand(FirstLdSt, OPU::OpName::vdst);
    SecondDst = getNamedOperand(SecondLdSt, OPU::OpName::vdata);
    if (!SecondDst)
      SecondDst = getNamedOperand(SecondLdSt, OPU::OpName::vdst);
  } else if (isSMRD(FirstLdSt) && isSMRD(SecondLdSt)) {
    FirstDst = getNamedOperand(FirstLdSt, OPU::OpName::sdst);
    SecondDst = getNamedOperand(SecondLdSt, OPU::OpName::sdst);
...
bool OPUInstrInfo::shouldScheduleLoadsNear(SDNode *Load0, SDNode *Load1,
...
static void reportIllegalCopy(const OPUInstrInfo *TII, MachineBasicBlock &MBB,
  ...
  BuildMI(MBB, MI, DL, TII->get(OPU::OPU_ILLEGAL_COPY), DestReg)
    .addReg(SrcReg, getKillRegState(KillSrc));
}

void OPUInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
                              MachineBasicBlock::iterator MI,
                              const DebugLoc &DL, unsigned DestReg,
                              unsigned SrcReg, bool KillSrc) const {
  const TargetRegisterClass *RC = RI.getPhysRegClass(DestReg);

  if (RC == &OPU::VGPR_32RegClass) {
    assert(OPU::VGPR_32RegClass.contains(SrcReg) ||
           OPU::SReg_32RegClass.contains(SrcReg) ||
           OPU::AGPR_32RegClass.contains(SrcReg));
    unsigned Opc = OPU::AGPR_32RegClass.contains(SrcReg) ?
                     OPU::V_ACCVGPR_READ_B32 : OPU::V_MOV_B32_e32;
    BuildMI(MBB, MI, DL, get(Opc), DestReg)
      .addReg(SrcReg, getKillRegState(KillSrc));
    return;
  }

  if (RC == &OPU::SReg_32_XM0RegClass ||
      RC == &OPU::SReg_32RegClass) {
    if (SrcReg == OPU::SCC) {
      BuildMI(MBB, MI, DL, get(OPU::S_CSELECT_B32), DestReg)
          .addImm(-1)
          .addImm(0);
      return;
    }

    if (DestReg == OPU::VCC_LO) {
      if (OPU::SReg_32RegClass.contains(SrcReg)) {
        BuildMI(MBB, MI, DL, get(OPU::S_MOV_B32), OPU::VCC_LO)
          .addReg(SrcReg, getKillRegState(KillSrc));
      } else {
        // FIXME: Hack until VReg_1 removed.
        assert(OPU::VGPR_32RegClass.contains(SrcReg));
        BuildMI(MBB, MI, DL, get(OPU::V_CMP_NE_U32_e32))
          .addImm(0)
          .addReg(SrcReg, getKillRegState(KillSrc));
      }
    ...
    BuildMI(MBB, MI, DL, get(OPU::S_MOV_B32), DestReg)
            .addReg(SrcReg, getKillRegState(KillSrc));
    return;
  }

  if (RC == &OPU::SReg_64RegClass) {
    if (DestReg == OPU::VCC) {
      if (OPU::SReg_64RegClass.contains(SrcReg)) {
        BuildMI(MBB, MI, DL, get(OPU::S_MOV_B64), OPU::VCC)
          .addReg(SrcReg, getKillRegState(KillSrc));
      } else {
        // FIXME: Hack until VReg_1 removed.
        assert(OPU::VGPR_32RegClass.contains(SrcReg));
    ...
    BuildMI(MBB, MI, DL, get(OPU::S_MOV_B64), DestReg)
            .addReg(SrcReg, getKillRegState(KillSrc));
    return;
  }

  if (DestReg == OPU::SCC) {
    assert(OPU::SReg_32RegClass.contains(SrcReg));
    BuildMI(MBB, MI, DL, get(OPU::S_CMP_LG_U32))
      .addReg(SrcReg, getKillRegState(KillSrc))
      .addImm(0);
    return;
  }

  if (RC == &OPU::AGPR_32RegClass) {
    assert(OPU::VGPR_32RegClass.contains(SrcReg) ||
           OPU::SReg_32RegClass.contains(SrcReg) ||
           OPU::AGPR_32RegClass.contains(SrcReg));
    if (!OPU::VGPR_32RegClass.contains(SrcReg)) {
      ...
      for (auto Def = MI, E = MBB.begin(); Def != E; ) {
        ...
        if (!Def->definesRegister(SrcReg, &RI))
          continue;
        if (Def->getOpcode() != OPU::V_ACCVGPR_WRITE_B32)
          break;
        ...
        BuildMI(MBB, MI, DL, get(OPU::V_ACCVGPR_WRITE_B32), DestReg)
          .add(DefOp);
        return;
      }
      ...
      unsigned MaxVGPRs = RI.getRegPressureLimit(&OPU::VGPR_32RegClass,
                                                 *MBB.getParent());
      ...
      unsigned Tmp = RS.scavengeRegister(&OPU::VGPR_32RegClass, 0);
      if (!Tmp)
      ...
      while (RegNo-- && RS.FindUnusedReg(&OPU::VGPR_32RegClass)) {
        unsigned Tmp2 = RS.scavengeRegister(&OPU::VGPR_32RegClass, 0);
  ...
  if (RI.isSGPRClass(RC)) {
    ...
    Opcode = RI.hasVGPRs(RI.getPhysRegClass(SrcReg)) ?
      OPU::V_ACCVGPR_WRITE_B32 : OPU::COPY;
  } else if (RI.hasVGPRs(RC) && RI.hasAGPRs(RI.getPhysRegClass(SrcReg))) {
    Opcode = OPU::V_ACCVGPR_READ_B32;
  }
...
int OPUInstrInfo::commuteOpcode(unsigned Opcode) const {
  int NewOpc;

  // Try to map original to commuted opcode
  NewOpc = OPU::getCommuteRev(Opcode);
  if (NewOpc != -1)
    // Check if the commuted (REV) opcode exists on the target.
    return pseudoToMCOpcode(NewOpc) != -1 ? NewOpc : -1;

  // Try to map commuted to original opcode
  NewOpc = OPU::getCommuteOrig(Opcode);
  if (NewOpc != -1)
...
void OPUInstrInfo::materializeImmediate(MachineBasicBlock &MBB,
                                       MachineBasicBlock::iterator MI,
                                       const DebugLoc &DL, unsigned DestReg,
                                       int64_t Value) const {
  MachineRegisterInfo &MRI = MBB.getParent()->getRegInfo();
  const TargetRegisterClass *RegClass = MRI.getRegClass(DestReg);
  if (RegClass == &OPU::SReg_32RegClass ||
      RegClass == &OPU::SGPR_32RegClass ||
      RegClass == &OPU::SReg_32_XM0RegClass ||
      RegClass == &OPU::SReg_32_XM0_XEXECRegClass) {
    BuildMI(MBB, MI, DL, get(OPU::S_MOV_B32), DestReg)
      .addImm(Value);
    return;
  }

  if (RegClass == &OPU::SReg_64RegClass ||
      RegClass == &OPU::SGPR_64RegClass ||
      RegClass == &OPU::SReg_64_XEXECRegClass) {
    BuildMI(MBB, MI, DL, get(OPU::S_MOV_B64), DestReg)
      .addImm(Value);
    return;
  }

  if (RegClass == &OPU::VGPR_32RegClass) {
    BuildMI(MBB, MI, DL, get(OPU::V_MOV_B32_e32), DestReg)
      .addImm(Value);
    return;
  }
  if (RegClass == &OPU::VReg_64RegClass) {
    BuildMI(MBB, MI, DL, get(OPU::V_MOV_B64_PSEUDO), DestReg)
      .addImm(Value);
    return;
  }
...
OPUInstrInfo::getPreferredSelectRegClass(unsigned Size) const {
  return &OPU::VGPR_32RegClass;
}

void OPUInstrInfo::insertVectorSelect(MachineBasicBlock &MBB,
  ...
  const TargetRegisterClass *BoolXExecRC =
    RI.getRegClass(OPU::SReg_1_XEXECRegClassID);
  assert(MRI.getRegClass(DstReg) == &OPU::VGPR_32RegClass &&
         "Not a VGPR32 reg");

  if (Cond.size() == 1) {
    unsigned SReg = MRI.createVirtualRegister(BoolXExecRC);
    BuildMI(MBB, I, DL, get(OPU::COPY), SReg)
      .add(Cond[0]);
    BuildMI(MBB, I, DL, get(OPU::V_CNDMASK_B32_e64), DstReg)
      .addImm(0)
      .addReg(FalseReg)
      .addImm(0)
      .addReg(TrueReg)
    ...
    case OPUInstrInfo::SCC_TRUE: {
      unsigned SReg = MRI.createVirtualRegister(BoolXExecRC);
      BuildMI(MBB, I, DL, get(ST.isWave32() ? OPU::S_CSELECT_B32
                                            : OPU::S_CSELECT_B64), SReg)
        .addImm(-1)
        .addImm(0);
      BuildMI(MBB, I, DL, get(OPU::V_CNDMASK_B32_e64), DstReg)
        .addImm(0)
    ...
    case OPUInstrInfo::EXECNZ: {
      unsigned SReg = MRI.createVirtualRegister(BoolXExecRC);
      unsigned SReg2 = MRI.createVirtualRegister(RI.getBoolRC());
      BuildMI(MBB, I, DL, get(ST.isWave32() ? OPU::S_OR_SAVEEXEC_B32
                                            : OPU::S_OR_SAVEEXEC_B64), SReg2)
        .addImm(0);
      BuildMI(MBB, I, DL, get(ST.isWave32() ? OPU::S_CSELECT_B32
                                            : OPU::S_CSELECT_B64), SReg)
        .addImm(-1)
...
unsigned OPUInstrInfo::insertEQ(MachineBasicBlock *MBB,
                               MachineBasicBlock::iterator I,
                               const DebugLoc &DL,
                               unsigned SrcReg, int Value) const {
  MachineRegisterInfo &MRI = MBB->getParent()->getRegInfo();
  unsigned Reg = MRI.createVirtualRegister(RI.getBoolRC());
  BuildMI(*MBB, I, DL, get(OPU::V_CMP_EQ_I32_e64), Reg)
    .addImm(Value)
    .addReg(SrcReg);
...
unsigned OPUInstrInfo::insertNE(MachineBasicBlock *MBB,
                               MachineBasicBlock::iterator I,
                               const DebugLoc &DL,
                               unsigned SrcReg, int Value) const {
  MachineRegisterInfo &MRI = MBB->getParent()->getRegInfo();
  unsigned Reg = MRI.createVirtualRegister(RI.getBoolRC());
  BuildMI(*MBB, I, DL, get(OPU::V_CMP_NE_I32_e64), Reg)
    .addImm(Value)
    .addReg(SrcReg);
...
unsigned OPUInstrInfo::getMovOpcode(const TargetRegisterClass *DstRC) const {

  if (RI.hasAGPRs(DstRC))
    return OPU::COPY;
  if (RI.getRegSizeInBits(*DstRC) == 32) {
    return RI.isSGPRClass(DstRC) ? OPU::S_MOV_B32 : OPU::V_MOV_B32_e32;
  } else if (RI.getRegSizeInBits(*DstRC) == 64 && RI.isSGPRClass(DstRC)) {
    return OPU::S_MOV_B64;
  } else if (RI.getRegSizeInBits(*DstRC) == 64 && !RI.isSGPRClass(DstRC)) {
    return  OPU::V_MOV_B64_PSEUDO;
  }
  return OPU::COPY;
}
...
static unsigned getSGPRSpillSaveOpcode(unsigned Size) {
  switch (Size) {
  case 4:
    return OPU::OPU_SPILL_S32_SAVE;
  case 8:
    return OPU::OPU_SPILL_S64_SAVE;
  case 12:
    return OPU::OPU_SPILL_S96_SAVE;
  case 16:
    return OPU::OPU_SPILL_S128_SAVE;
  case 20:
    return OPU::OPU_SPILL_S160_SAVE;
  case 32:
    return OPU::OPU_SPILL_S256_SAVE;
  case 64:
    return OPU::OPU_SPILL_S512_SAVE;
  case 128:
    return OPU::OPU_SPILL_S1024_SAVE;
  default:
...
static unsigned getVGPRSpillSaveOpcode(unsigned Size) {
  switch (Size) {
  case 4:
    return OPU::OPU_SPILL_V32_SAVE;
  case 8:
    return OPU::OPU_SPILL_V64_SAVE;
  case 12:
    return OPU::OPU_SPILL_V96_SAVE;
  case 16:
    return OPU::OPU_SPILL_V128_SAVE;
  case 20:
    return OPU::OPU_SPILL_V160_SAVE;
  case 32:
    return OPU::OPU_SPILL_V256_SAVE;
  case 64:
    return OPU::OPU_SPILL_V512_SAVE;
  case 128:
    return OPU::OPU_SPILL_V1024_SAVE;
  default:
    llvm_unreachable("unknown register size");
  }
}
...
static unsigned getAGPRSpillSaveOpcode(unsigned Size) {
  switch (Size) {
  case 4:
    return OPU::OPU_SPILL_A32_SAVE;
  case 8:
    return OPU::OPU_SPILL_A64_SAVE;
  case 16:
    return OPU::OPU_SPILL_A128_SAVE;
  case 64:
    return OPU::OPU_SPILL_A512_SAVE;
  case 128:
    return OPU::OPU_SPILL_A1024_SAVE;
  default:
    llvm_unreachable("unknown register size");
  }
}

void OPUInstrInfo::storeRegToStackSlot(MachineBasicBlock &MBB,
                                      MachineBasicBlock::iterator MI,
  ...
  if (RI.isSGPRClass(RC)) {
    ...
    if (TargetRegisterInfo::isVirtualRegister(SrcReg) && SpillSize == 4) {
      ...
      MRI.constrainRegClass(SrcReg, &OPU::SReg_32_XM0RegClass);
    }
    ...
    if (ST.hasScalarStores()) {
      ...
      Spill.addReg(OPU::M0, RegState::ImplicitDefine | RegState::Dead);
    }
  ...
  if (RI.hasAGPRs(RC)) {
    MachineRegisterInfo &MRI = MF->getRegInfo();
    unsigned Tmp = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);
...
static unsigned getSGPRSpillRestoreOpcode(unsigned Size) {
  switch (Size) {
  case 4:
    return OPU::OPU_SPILL_S32_RESTORE;
  case 8:
    return OPU::OPU_SPILL_S64_RESTORE;
  case 12:
    return OPU::OPU_SPILL_S96_RESTORE;
  case 16:
    return OPU::OPU_SPILL_S128_RESTORE;
  case 20:
    return OPU::OPU_SPILL_S160_RESTORE;
  case 32:
    return OPU::OPU_SPILL_S256_RESTORE;
  case 64:
    return OPU::OPU_SPILL_S512_RESTORE;
  case 128:
    return OPU::OPU_SPILL_S1024_RESTORE;
  default:
    llvm_unreachable("unknown register size");
  }
}
...
static unsigned getVGPRSpillRestoreOpcode(unsigned Size) {
  switch (Size) {
  case 4:
    return OPU::OPU_SPILL_V32_RESTORE;
  case 8:
    return OPU::OPU_SPILL_V64_RESTORE;
  case 12:
    return OPU::OPU_SPILL_V96_RESTORE;
  case 16:
    return OPU::OPU_SPILL_V128_RESTORE;
  case 20:
    return OPU::OPU_SPILL_V160_RESTORE;
  case 32:
    return OPU::OPU_SPILL_V256_RESTORE;
  case 64:
    return OPU::OPU_SPILL_V512_RESTORE;
  case 128:
    return OPU::OPU_SPILL_V1024_RESTORE;
  default:
    llvm_unreachable("unknown register size");
  }
}
...
static unsigned getAGPRSpillRestoreOpcode(unsigned Size) {
  switch (Size) {
  case 4:
    return OPU::OPU_SPILL_A32_RESTORE;
  case 8:
    return OPU::OPU_SPILL_A64_RESTORE;
  case 16:
    return OPU::OPU_SPILL_A128_RESTORE;
  case 64:
    return OPU::OPU_SPILL_A512_RESTORE;
  case 128:
    return OPU::OPU_SPILL_A1024_RESTORE;
  default:
    llvm_unreachable("unknown register size");
  }
}

void OPUInstrInfo::loadRegFromStackSlot(MachineBasicBlock &MBB,
                                       MachineBasicBlock::iterator MI,
  ...
  if (RI.isSGPRClass(RC)) {
    ...
    if (TargetRegisterInfo::isVirtualRegister(DestReg) && SpillSize == 4) {
      MachineRegisterInfo &MRI = MF->getRegInfo();
      MRI.constrainRegClass(DestReg, &OPU::SReg_32_XM0RegClass);
    }
...
unsigned OPUInstrInfo::calculateLDSSpillAddress(
  ...
  if (!MFI->hasCalculatedTID()) {
    ...
    TIDReg = RI.findUnusedRegister(MF->getRegInfo(), &OPU::VGPR_32RegClass,
                                   *MF);
    if (TIDReg == OPU::NoRegister)
      return TIDReg;

    if (!OPU::isShader(MF->getFunction().getCallingConv()) &&
        WorkGroupSize > WavefrontSize) {
      unsigned TIDIGXReg
        = MFI->getPreloadedReg(OPUFunctionArgInfo::WORKGROUP_ID_X);
      unsigned TIDIGYReg
        = MFI->getPreloadedReg(OPUFunctionArgInfo::WORKGROUP_ID_Y);
      unsigned TIDIGZReg
        = MFI->getPreloadedReg(OPUFunctionArgInfo::WORKGROUP_ID_Z);
      ...
      unsigned STmp0 = RS->scavengeRegister(&OPU::SGPR_32RegClass, 0);
      unsigned STmp1 = RS->scavengeRegister(&OPU::SGPR_32RegClass, 0);
      BuildMI(Entry, Insert, DL, get(OPU::S_LOAD_DWORD_IMM), STmp0)
              .addReg(InputPtrReg)
              .addImm(OPU::KernelInputOffsets::NGROUPS_Z);
      BuildMI(Entry, Insert, DL, get(OPU::S_LOAD_DWORD_IMM), STmp1)
              .addReg(InputPtrReg)
              .addImm(OPU::KernelInputOffsets::NGROUPS_Y);

      // NGROUPS.X * NGROUPS.Y
      BuildMI(Entry, Insert, DL, get(OPU::S_MUL_I32), STmp1)
              .addReg(STmp1)
              .addReg(STmp0);
      // (NGROUPS.X * NGROUPS.Y) * TIDIG.X
      BuildMI(Entry, Insert, DL, get(OPU::V_MUL_U32_U24_e32), TIDReg)
              .addReg(STmp1)
              .addReg(TIDIGXReg);
      // NGROUPS.Z * TIDIG.Y + (NGROUPS.X * NGROPUS.Y * TIDIG.X)
      BuildMI(Entry, Insert, DL, get(OPU::V_MAD_U32_U24), TIDReg)
              .addReg(STmp0)
              .addReg(TIDIGYReg)
              .addReg(TIDReg);
      ...
      // Get the wave id
      BuildMI(Entry, Insert, DL, get(OPU::V_MBCNT_LO_U32_B32_e64),
              TIDReg)
              .addImm(-1)
              .addImm(0);

      BuildMI(Entry, Insert, DL, get(OPU::V_MBCNT_HI_U32_B32_e64),
              TIDReg)
              .addImm(-1)
              .addReg(TIDReg);
    }

    BuildMI(Entry, Insert, DL, get(OPU::V_LSHLREV_B32_e32),
            TIDReg)
            .addImm(2)
            .addReg(TIDReg);
...
void OPUInstrInfo::insertWaitStates(MachineBasicBlock &MBB,
                                   MachineBasicBlock::iterator MI,
                                   int Count) const {
  ...
  while (Count > 0) {
    ...
    if (Count >= 8)
      ...
      Arg = Count - 1;
    Count -= 8;
    BuildMI(MBB, MI, DL, get(OPU::S_NOP))
            .addImm(Arg);
  }
}

void OPUInstrInfo::insertNoop(MachineBasicBlock &MBB,
                             MachineBasicBlock::iterator MI) const {
  insertWaitStates(MBB, MI, 1);
}

void OPUInstrInfo::insertReturn(MachineBasicBlock &MBB) const {
  ...
  if (MBB.succ_empty()) {
    ...
    if (HasNoTerminator) {
      if (Info->returnsVoid()) {
        BuildMI(MBB, MBB.end(), DebugLoc(), get(OPU::S_ENDPGM)).addImm(0);
      } else {
        BuildMI(MBB, MBB.end(), DebugLoc(), get(OPU::OPU_RETURN_TO_EPILOG));
      }
    }
  }
}

unsigned OPUInstrInfo::getNumWaitStates(const MachineInstr &MI) {
  switch (MI.getOpcode()) {
...
bool OPUInstrInfo::expandPostRAPseudo(MachineInstr &MI) const {
  MachineBasicBlock &MBB = *MI.getParent();
  DebugLoc DL = MBB.findDebugLoc(MI);
  switch (MI.getOpcode()) {
  default: return TargetInstrInfo::expandPostRAPseudo(MI);
  case OPU::S_MOV_B64_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(OPU::S_MOV_B64));
    break;

  case OPU::S_MOV_B32_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(OPU::S_MOV_B32));
    break;

  case OPU::S_XOR_B64_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(OPU::S_XOR_B64));
    break;

  case OPU::S_XOR_B32_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(OPU::S_XOR_B32));
    break;

  case OPU::S_OR_B32_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(OPU::S_OR_B32));
    break;

  case OPU::S_ANDN2_B64_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(OPU::S_ANDN2_B64));
    break;

  case OPU::S_ANDN2_B32_term:
    // This is only a terminator to get the correct spill code placement during
    // register allocation.
    MI.setDesc(get(OPU::S_ANDN2_B32));
    break;

  case OPU::V_MOV_B64_PSEUDO: {
    unsigned Dst = MI.getOperand(0).getReg();
    unsigned DstLo = RI.getSubReg(Dst, OPU::sub0);
    unsigned DstHi = RI.getSubReg(Dst, OPU::sub1);
    ...
    if (SrcOp.isImm()) {
      ...
      BuildMI(MBB, MI, DL, get(OPU::V_MOV_B32_e32), DstLo)
        .addImm(Imm.getLoBits(32).getZExtValue())
        .addReg(Dst, RegState::Implicit | RegState::Define);
      BuildMI(MBB, MI, DL, get(OPU::V_MOV_B32_e32), DstHi)
        .addImm(Imm.getHiBits(32).getZExtValue())
        .addReg(Dst, RegState::Implicit | RegState::Define);
    } else {
      assert(SrcOp.isReg());
      BuildMI(MBB, MI, DL, get(OPU::V_MOV_B32_e32), DstLo)
        .addReg(RI.getSubReg(SrcOp.getReg(), OPU::sub0))
        .addReg(Dst, RegState::Implicit | RegState::Define);
      BuildMI(MBB, MI, DL, get(OPU::V_MOV_B32_e32), DstHi)
        .addReg(RI.getSubReg(SrcOp.getReg(), OPU::sub1))
        .addReg(Dst, RegState::Implicit | RegState::Define);
    }
    MI.eraseFromParent();
    break;
  }
  case OPU::V_SET_INACTIVE_B32: {
    unsigned NotOpc = ST.isWave32() ? OPU::S_NOT_B32 : OPU::S_NOT_B64;
    unsigned Exec = ST.isWave32() ? OPU::EXEC_LO : OPU::EXEC;
    BuildMI(MBB, MI, DL, get(NotOpc), Exec)
      .addReg(Exec);
    BuildMI(MBB, MI, DL, get(OPU::V_MOV_B32_e32), MI.getOperand(0).getReg())
      .add(MI.getOperand(2));
    BuildMI(MBB, MI, DL, get(NotOpc), Exec)
      .addReg(Exec);
    MI.eraseFromParent();
  ...
  case OPU::V_SET_INACTIVE_B64: {
    unsigned NotOpc = ST.isWave32() ? OPU::S_NOT_B32 : OPU::S_NOT_B64;
    unsigned Exec = ST.isWave32() ? OPU::EXEC_LO : OPU::EXEC;
    BuildMI(MBB, MI, DL, get(NotOpc), Exec)
      .addReg(Exec);
    MachineInstr *Copy = BuildMI(MBB, MI, DL, get(OPU::V_MOV_B64_PSEUDO),
  ...
  case OPU::V_MOVRELD_B32_V1:
  case OPU::V_MOVRELD_B32_V2:
  case OPU::V_MOVRELD_B32_V4:
  case OPU::V_MOVRELD_B32_V8:
  case OPU::V_MOVRELD_B32_V16: {
    const MCInstrDesc &MovRelDesc = get(OPU::V_MOVRELD_B32_e32);
    unsigned VecReg = MI.getOperand(0).getReg();
    bool IsUndef = MI.getOperand(1).isUndef();
    unsigned SubReg = OPU::sub0 + MI.getOperand(3).getImm();
  ...
  case OPU::OPU_PC_ADD_REL_OFFSET: {
    MachineFunction &MF = *MBB.getParent();
    unsigned Reg = MI.getOperand(0).getReg();
    unsigned RegLo = RI.getSubReg(Reg, OPU::sub0);
    unsigned RegHi = RI.getSubReg(Reg, OPU::sub1);

    // Create a bundle so these instructions won't be re-ordered by the
    // post-RA scheduler.
    MIBundleBuilder Bundler(MBB, MI);
    Bundler.append(BuildMI(MF, DL, get(OPU::S_GETPC_B64), Reg));

    // Add 32-bit offset from this instruction to the start of the
    // constant data.
    Bundler.append(BuildMI(MF, DL, get(OPU::S_ADD_U32), RegLo)
                       .addReg(RegLo)
                       .add(MI.getOperand(1)));

    MachineInstrBuilder MIB = BuildMI(MF, DL, get(OPU::S_ADDC_U32), RegHi)
                                  .addReg(RegHi);
  ...
  case OPU::ENTER_WWM: {
    // This only gets its own opcode so that OPUPreAllocateWWMRegs can tell when
    // WWM is entered.
    MI.setDesc(get(ST.isWave32() ? OPU::S_OR_SAVEEXEC_B32
                                 : OPU::S_OR_SAVEEXEC_B64));
    break;
  }
  case OPU::EXIT_WWM: {
    // This only gets its own opcode so that OPUPreAllocateWWMRegs can tell when
    // WWM is exited.
    MI.setDesc(get(ST.isWave32() ? OPU::S_MOV_B32 : OPU::S_MOV_B64));
    break;
  }
...
bool OPUInstrInfo::swapSourceModifiers(MachineInstr &MI,
                                      MachineOperand &Src0,
                                      unsigned Src0OpName,
                                      MachineOperand &Src1,
...
MachineInstr *OPUInstrInfo::commuteInstructionImpl(MachineInstr &MI, bool NewMI,
                                                  unsigned Src0Idx,
  ...
  assert(OPU::getNamedOperandIdx(Opc, OPU::OpName::src0) ==
           static_cast<int>(Src0Idx) &&
         OPU::getNamedOperandIdx(Opc, OPU::OpName::src1) ==
  ...
  if (CommutedMI) {
    swapSourceModifiers(MI, Src0, OPU::OpName::src0_modifiers,
                        Src1, OPU::OpName::src1_modifiers);
...
bool OPUInstrInfo::findCommutedOpIndices(MachineInstr &MI, unsigned &SrcOpIdx0,
                                        unsigned &SrcOpIdx1) const {
  return findCommutedOpIndices(MI.getDesc(), SrcOpIdx0, SrcOpIdx1);
}

bool OPUInstrInfo::findCommutedOpIndices(MCInstrDesc Desc, unsigned &SrcOpIdx0,
                                        unsigned &SrcOpIdx1) const {
...
bool OPUInstrInfo::isBranchOffsetInRange(unsigned BranchOp,
                                        int64_t BrOffset) const {
  // BranchRelaxation should never have to check s_setpc_b64 because its dest
  // block is unanalyzable.
  assert(BranchOp != OPU::S_SETPC_B64);

  // Convert to dwords.
  BrOffset /= 4;

  // The branch instructions do PC += signext(OPUMM16 * 4) + 4, so the offset is
...
MachineBasicBlock *OPUInstrInfo::getBranchDestBlock(
  const MachineInstr &MI) const {
  if (MI.getOpcode() == OPU::S_SETPC_B64) {
...
unsigned OPUInstrInfo::insertIndirectBranch(MachineBasicBlock &MBB,
  ...
  // blocks.
  unsigned PCReg = MRI.createVirtualRegister(&OPU::SReg_64RegClass);
  ...
  MachineInstr *GetPC = BuildMI(MBB, I, DL, get(OPU::S_GETPC_B64), PCReg);

  // TODO: Handle > 32-bit block address.
  if (BrOffset >= 0) {
    BuildMI(MBB, I, DL, get(OPU::S_ADD_U32))
      .addReg(PCReg, RegState::Define, OPU::sub0)
      .addReg(PCReg, 0, OPU::sub0)
      .addMBB(&DestBB, MO_LONG_BRANCH_FORWARD);
    BuildMI(MBB, I, DL, get(OPU::S_ADDC_U32))
      .addReg(PCReg, RegState::Define, OPU::sub1)
      .addReg(PCReg, 0, OPU::sub1)
      .addImm(0);
  } else {
    // Backwards branch.
    BuildMI(MBB, I, DL, get(OPU::S_SUB_U32))
      .addReg(PCReg, RegState::Define, OPU::sub0)
      .addReg(PCReg, 0, OPU::sub0)
      .addMBB(&DestBB, MO_LONG_BRANCH_BACKWARD);
    BuildMI(MBB, I, DL, get(OPU::S_SUBB_U32))
      .addReg(PCReg, RegState::Define, OPU::sub1)
      .addReg(PCReg, 0, OPU::sub1)
      .addImm(0);
  }
  ...
  BuildMI(&MBB, DL, get(OPU::S_SETPC_B64))
    .addReg(PCReg);
...
unsigned OPUInstrInfo::getBranchOpcode(OPUInstrInfo::BranchPredicate Cond) {
  switch (Cond) {
  case OPUInstrInfo::SCC_TRUE:
    return OPU::S_CBRANCH_SCC1;
  case OPUInstrInfo::SCC_FALSE:
    return OPU::S_CBRANCH_SCC0;
  case OPUInstrInfo::VCCNZ:
    return OPU::S_CBRANCH_VCCNZ;
  case OPUInstrInfo::VCCZ:
    return OPU::S_CBRANCH_VCCZ;
  case OPUInstrInfo::EXECNZ:
    return OPU::S_CBRANCH_EXECNZ;
  case OPUInstrInfo::EXECZ:
    return OPU::S_CBRANCH_EXECZ;
  default:
    llvm_unreachable("invalid branch predicate");
  }
}

OPUInstrInfo::BranchPredicate OPUInstrInfo::getBranchPredicate(unsigned Opcode) {
  switch (Opcode) {
  case OPU::S_CBRANCH_SCC0:
    return SCC_FALSE;
  case OPU::S_CBRANCH_SCC1:
    return SCC_TRUE;
  case OPU::S_CBRANCH_VCCNZ:
    return VCCNZ;
  case OPU::S_CBRANCH_VCCZ:
    return VCCZ;
  case OPU::S_CBRANCH_EXECNZ:
    return EXECNZ;
  case OPU::S_CBRANCH_EXECZ:
    return EXECZ;
  default:
    return INVALID_BR;
  }
...
bool OPUInstrInfo::analyzeBranchImpl(MachineBasicBlock &MBB,
                                    MachineBasicBlock::iterator I,
                                    MachineBasicBlock *&TBB,
                                    MachineBasicBlock *&FBB,
                                    SmallVectorImpl<MachineOperand> &Cond,
                                    bool AllowModify) const {
  if (I->getOpcode() == OPU::S_BRANCH) {
  ...
  if (I->getOpcode() == OPU::OPU_NON_UNIFORM_BRCOND_PSEUDO) {
...
bool OPUInstrInfo::analyzeBranch(MachineBasicBlock &MBB, MachineBasicBlock *&TBB,
                                MachineBasicBlock *&FBB,
                                SmallVectorImpl<MachineOperand> &Cond,
                                bool AllowModify) const {
  ...
  while (I != E && !I->isBranch() && !I->isReturn() &&
         I->getOpcode() != OPU::OPU_MASK_BRANCH) {
    switch (I->getOpcode()) {
    case OPU::OPU_MASK_BRANCH:
    case OPU::S_MOV_B64_term:
    case OPU::S_XOR_B64_term:
    case OPU::S_ANDN2_B64_term:
    case OPU::S_MOV_B32_term:
    ...
    case OPU::OPU_IF:
    case OPU::OPU_ELSE:
    case OPU::OPU_KILL_I1_TERMINATOR:
    case OPU::OPU_KILL_F32_COND_IMM_TERMINATOR:
  ...
  if (I->getOpcode() != OPU::OPU_MASK_BRANCH)
...
unsigned OPUInstrInfo::removeBranch(MachineBasicBlock &MBB,
...
unsigned OPUInstrInfo::insertBranch(MachineBasicBlock &MBB,
                                   MachineBasicBlock *TBB,
                                   MachineBasicBlock *FBB,
                                   ArrayRef<MachineOperand> Cond,
                                   const DebugLoc &DL,
  ...
  if (!FBB && Cond.empty()) {
    BuildMI(&MBB, DL, get(OPU::S_BRANCH))
      .addMBB(TBB);
    if (BytesAdded)
      *BytesAdded = 4;
    return 1;
  ...
  if(Cond.size() == 1 && Cond[0].isReg()) {
     BuildMI(&MBB, DL, get(OPU::OPU_NON_UNIFORM_BRCOND_PSEUDO))
       .add(Cond[0])
       .addMBB(TBB);
     return 1;
  }
...
bool OPUInstrInfo::reverseBranchCondition(
...
bool OPUInstrInfo::canInsertSelect(const MachineBasicBlock &MBB,
                                  ArrayRef<MachineOperand> Cond,
  ...
  case VCCZ: {
    ...
    int NumInsts = OPU::getRegBitWidth(RC->getID()) / 32;
...
void OPUInstrInfo::insertSelect(MachineBasicBlock &MBB,
  ...
  if (DstSize == 32) {
    unsigned SelOp = Pred == SCC_TRUE ?
      OPU::S_CSELECT_B32 : OPU::V_CNDMASK_B32_e32;
  ...
  if (DstSize == 64 && Pred == SCC_TRUE) {
    MachineInstr *Select =
      BuildMI(MBB, I, DL, get(OPU::S_CSELECT_B64), DstReg)
      .addReg(FalseReg)
      .addReg(TrueReg);
  ...
  unsigned SelOp = OPU::V_CNDMASK_B32_e32;
  const TargetRegisterClass *EltRC = &OPU::VGPR_32RegClass;
  ...
  if (Pred == SCC_TRUE) {
    if (NElts % 2) {
      SelOp = OPU::S_CSELECT_B32;
      EltRC = &OPU::SGPR_32RegClass;
    } else {
      SelOp = OPU::S_CSELECT_B64;
      EltRC = &OPU::SGPR_64RegClass;
  ...
  MachineInstrBuilder MIB = BuildMI(
    MBB, I, DL, get(OPU::REG_SEQUENCE), DstReg);
...
bool OPUInstrInfo::isFoldableCopy(const MachineInstr &MI) const {
  switch (MI.getOpcode()) {
  case OPU::V_MOV_B32_e32:
  case OPU::V_MOV_B32_e64:
  case OPU::V_MOV_B64_PSEUDO: {
  ...
  case OPU::S_MOV_B64:
  case OPU::COPY:
  case OPU::V_ACCVGPR_WRITE_B32:
  case OPU::V_ACCVGPR_READ_B32:
    return true;
  default:
    return false;
  }
...
unsigned OPUInstrInfo::getAddressSpaceForPseudoSourceKind(
    unsigned Kind) const {
  switch(Kind) {
  case PseudoSourceValue::Stack:
  case PseudoSourceValue::FixedStack:
    return OPUAS::PRIVATE_ADDRESS;
  ...
  case PseudoSourceValue::TargetCustom:
    return OPUAS::CONSTANT_ADDRESS;
  }
  return OPUAS::FLAT_ADDRESS;
}

static void removeModOperands(MachineInstr &MI) {
  unsigned Opc = MI.getOpcode();
  int Src0ModIdx = OPU::getNamedOperandIdx(Opc,
                                              OPU::OpName::src0_modifiers);
  int Src1ModIdx = OPU::getNamedOperandIdx(Opc,
                                              OPU::OpName::src1_modifiers);
  int Src2ModIdx = OPU::getNamedOperandIdx(Opc,
...
bool OPUInstrInfo::FoldImmediate(MachineInstr &UseMI, MachineInstr &DefMI,
  ...
  const MachineOperand *ImmOp = getNamedOperand(DefMI, OPU::OpName::src0);
  assert(ImmOp);
  ...
  if (Opc == OPU::COPY) {
    ...
    unsigned NewOpc = isVGPRCopy ? OPU::V_MOV_B32_e32 : OPU::S_MOV_B32;
    if (RI.isAGPR(*MRI, UseMI.getOperand(0).getReg())) {
      if (!isInlineConstant(*ImmOp, OPU::OPERAND_REG_INLINE_AC_INT32))
        return false;
      NewOpc = OPU::V_ACCVGPR_WRITE_B32;
    }
  ...
  if (Opc == OPU::V_MAD_F32 || Opc == OPU::V_MAC_F32_e64 ||
      Opc == OPU::V_MAD_F16 || Opc == OPU::V_MAC_F16_e64 ||
      Opc == OPU::V_FMA_F32 || Opc == OPU::V_FMAC_F32_e64 ||
      Opc == OPU::V_FMA_F16 || Opc == OPU::V_FMAC_F16_e64) {
    ...
    // TODO: We could fold this here instead of letting OPUFoldOperands do it
    // later.
    MachineOperand *Src0 = getNamedOperand(UseMI, OPU::OpName::src0);

    // Any src operand can be used for the legality check.
    if (isInlineConstant(UseMI, *Src0, *ImmOp))
      return false;

    bool IsF32 = Opc == OPU::V_MAD_F32 || Opc == OPU::V_MAC_F32_e64 ||
                 Opc == OPU::V_FMA_F32 || Opc == OPU::V_FMAC_F32_e64;
    bool IsFMA = Opc == OPU::V_FMA_F32 || Opc == OPU::V_FMAC_F32_e64 ||
                 Opc == OPU::V_FMA_F16 || Opc == OPU::V_FMAC_F16_e64;
    MachineOperand *Src1 = getNamedOperand(UseMI, OPU::OpName::src1);
    MachineOperand *Src2 = getNamedOperand(UseMI, OPU::OpName::src2);
    ...
    if (Src0->isReg() && Src0->getReg() == Reg) {
      ...
      unsigned NewOpc =
        IsFMA ? (IsF32 ? OPU::V_FMAMK_F32 : OPU::V_FMAMK_F16)
              : (IsF32 ? OPU::V_MADMK_F32 : OPU::V_MADMK_F16);
      ...
      UseMI.RemoveOperand(
          OPU::getNamedOperandIdx(Opc, OPU::OpName::omod));
      UseMI.RemoveOperand(
          OPU::getNamedOperandIdx(Opc, OPU::OpName::clamp));
      ...
      if (Opc == OPU::V_MAC_F32_e64 ||
          Opc == OPU::V_MAC_F16_e64 ||
          Opc == OPU::V_FMAC_F32_e64 ||
          Opc == OPU::V_FMAC_F16_e64)
        UseMI.untieRegOperand(
            OPU::getNamedOperandIdx(Opc, OPU::OpName::src2));
    ...
    if (Src2->isReg() && Src2->getReg() == Reg) {
      ...
      unsigned NewOpc =
        IsFMA ? (IsF32 ? OPU::V_FMAAK_F32 : OPU::V_FMAAK_F16)
              : (IsF32 ? OPU::V_MADAK_F32 : OPU::V_MADAK_F16);
      if (pseudoToMCOpcode(NewOpc) == -1)
        return false;
...
bool OPUInstrInfo::checkInstOffsetsDoNotOverlap(const MachineInstr &MIa,
...
bool OPUInstrInfo::areMemAccessesTriviallyDisjoint(const MachineInstr &MIa,
...
static int64_t getFoldableImm(const MachineOperand* MO) {
  ...
  if (Def && Def->getOpcode() == OPU::V_MOV_B32_e32 &&
      Def->getOperand(1).isImm())
    return Def->getOperand(1).getImm();
  return OPU::NoRegister;
}

MachineInstr *OPUInstrInfo::convertToThreeAddress(MachineFunction::iterator &MBB,
                                                 MachineInstr &MI,
                                                 LiveVariables *LV) const {
  unsigned Opc = MI.getOpcode();
  bool IsF16 = false;
  bool IsFMA = Opc == OPU::V_FMAC_F32_e32 || Opc == OPU::V_FMAC_F32_e64 ||
               Opc == OPU::V_FMAC_F16_e32 || Opc == OPU::V_FMAC_F16_e64;

  switch (Opc) {
  default:
    return nullptr;
  case OPU::V_MAC_F16_e64:
  case OPU::V_FMAC_F16_e64:
    IsF16 = true;
    LLVM_FALLTHROUGH;
  case OPU::V_MAC_F32_e64:
  case OPU::V_FMAC_F32_e64:
    break;
  case OPU::V_MAC_F16_e32:
  case OPU::V_FMAC_F16_e32:
    IsF16 = true;
    LLVM_FALLTHROUGH;
  case OPU::V_MAC_F32_e32:
  case OPU::V_FMAC_F32_e32: {
    int Src0Idx = OPU::getNamedOperandIdx(MI.getOpcode(),
                                             OPU::OpName::src0);
  ...
  const MachineOperand *Dst = getNamedOperand(MI, OPU::OpName::vdst);
  const MachineOperand *Src0 = getNamedOperand(MI, OPU::OpName::src0);
  const MachineOperand *Src0Mods =
    getNamedOperand(MI, OPU::OpName::src0_modifiers);
  const MachineOperand *Src1 = getNamedOperand(MI, OPU::OpName::src1);
  const MachineOperand *Src1Mods =
    getNamedOperand(MI, OPU::OpName::src1_modifiers);
  const MachineOperand *Src2 = getNamedOperand(MI, OPU::OpName::src2);
  const MachineOperand *Clamp = getNamedOperand(MI, OPU::OpName::clamp);
  const MachineOperand *Omod = getNamedOperand(MI, OPU::OpName::omod);
  ...
  if (!Src0Mods && !Src1Mods && !Clamp && !Omod &&
    ...
    if (auto Imm = getFoldableImm(Src2)) {
      unsigned NewOpc =
         IsFMA ? (IsF16 ? OPU::V_FMAAK_F16 : OPU::V_FMAAK_F32)
               : (IsF16 ? OPU::V_MADAK_F16 : OPU::V_MADAK_F32);
      if (pseudoToMCOpcode(NewOpc) != -1)
    ...
    unsigned NewOpc =
      IsFMA ? (IsF16 ? OPU::V_FMAMK_F16 : OPU::V_FMAMK_F32)
            : (IsF16 ? OPU::V_MADMK_F16 : OPU::V_MADMK_F32);
    ...
    if (auto Imm = getFoldableImm(Src0)) {
      if (pseudoToMCOpcode(NewOpc) != -1 &&
          isOperandLegal(MI, OPU::getNamedOperandIdx(NewOpc,
                           OPU::OpName::src0), Src1))
        return BuildMI(*MBB, MI, MI.getDebugLoc(), get(NewOpc))
                 .add(*Dst)
                 .add(*Src1)
                 .addImm(Imm)
  ...
  unsigned NewOpc = IsFMA ? (IsF16 ? OPU::V_FMA_F16 : OPU::V_FMA_F32)
                          : (IsF16 ? OPU::V_MAD_F16 : OPU::V_MAD_F32);
  if (pseudoToMCOpcode(NewOpc) == -1)
    return nullptr;
...
static bool changesVGPRIndexingMode(const MachineInstr &MI) {
  switch (MI.getOpcode()) {
  case OPU::S_SET_GPR_IDX_ON:
  case OPU::S_SET_GPR_IDX_MODE:
  case OPU::S_SET_GPR_IDX_OFF:
    return true;
  default:
    return false;
  }
...
bool OPUInstrInfo::isSchedulingBoundary(const MachineInstr &MI,
  ...
  return TargetInstrInfo::isSchedulingBoundary(MI, MBB, MF) ||
         MI.modifiesRegister(OPU::EXEC, &RI) ||
         MI.getOpcode() == OPU::S_SETREG_IMM32_B32 ||
         MI.getOpcode() == OPU::S_SETREG_B32 ||
         changesVGPRIndexingMode(MI);
}

bool OPUInstrInfo::isAlwaysGDS(uint16_t Opcode) const {
  return Opcode == OPU::DS_ORDERED_COUNT ||
         Opcode == OPU::DS_GWS_INIT ||
         Opcode == OPU::DS_GWS_SEMA_V ||
         Opcode == OPU::DS_GWS_SEMA_BR ||
         Opcode == OPU::DS_GWS_SEMA_P ||
         Opcode == OPU::DS_GWS_SEMA_RELEASE_ALL ||
         Opcode == OPU::DS_GWS_BARRIER;
}

bool OPUInstrInfo::hasUnwantedEffectsWhenEXECEmpty(const MachineInstr &MI) const {
  ...
  if (Opcode == OPU::S_SENDMSG || Opcode == OPU::S_SENDMSGHALT ||
      Opcode == OPU::EXP || Opcode == OPU::EXP_DONE ||
      Opcode == OPU::DS_ORDERED_COUNT || Opcode == OPU::S_TRAP ||
      Opcode == OPU::DS_GWS_INIT || Opcode == OPU::DS_GWS_BARRIER)
    return true;
  ...
  if (Opcode == OPU::V_READFIRSTLANE_B32 || Opcode == OPU::V_READLANE_B32)
    return true;

  return false;
}

bool OPUInstrInfo::mayReadEXEC(const MachineRegisterInfo &MRI,
                              const MachineInstr &MI) const {
  ...
  if (MI.isCopyLike()) {
    ...
    return MI.readsRegister(OPU::EXEC, &RI);
  }
  ...
  return !isSALU(MI) || MI.readsRegister(OPU::EXEC, &RI);
}

bool OPUInstrInfo::isInlineConstant(const APInt &Imm) const {
  switch (Imm.getBitWidth()) {
  case 1: // This likely will be a condition code mask.
    return true;

  case 32:
    return OPU::isInlinableLiteral32(Imm.getSExtValue(),
                                        ST.hasInv2PiInlineImm());
  case 64:
    return OPU::isInlinableLiteral64(Imm.getSExtValue(),
                                        ST.hasInv2PiInlineImm());
  case 16:
    return ST.has16BitInsts() &&
           OPU::isInlinableLiteral16(Imm.getSExtValue(),
                                        ST.hasInv2PiInlineImm());
  default:
...
bool OPUInstrInfo::isInlineConstant(const MachineOperand &MO,
                                   uint8_t OperandType) const {
  if (!MO.isImm() ||
      OperandType < OPU::OPERAND_SRC_FIRST ||
      OperandType > OPU::OPERAND_SRC_LAST)
    return false;
  ...
  case OPU::OPERAND_REG_INLINE_AC_FP32: {
    ...
    return OPU::isInlinableLiteral32(Trunc, ST.hasInv2PiInlineImm());
  }
  ...
  case OPU::OPERAND_REG_INLINE_C_FP64:
    return OPU::isInlinableLiteral64(MO.getImm(),
                                        ST.hasInv2PiInlineImm());
  ...
  case OPU::OPERAND_REG_INLINE_AC_FP16: {
    if (isInt<16>(Imm) || isUInt<16>(Imm)) {
      ...
      return ST.has16BitInsts() &&
             OPU::isInlinableLiteral16(Trunc, ST.hasInv2PiInlineImm());
    }
  ...
  case OPU::OPERAND_REG_INLINE_AC_V2FP16: {
    ...
    return OPU::isInlinableLiteralV216(Trunc, ST.hasInv2PiInlineImm());
  }
  default:
    llvm_unreachable("invalid bitwidth");
  }
...
bool OPUInstrInfo::isLiteralConstantLike(const MachineOperand &MO,
...
bool OPUInstrInfo::isImmOperandLegal(const MachineInstr &MI, unsigned OpNo,
  ...
  if (!isVOP3(MI) || !OPU::isOPUSrcOperand(InstDesc, OpNo))
    return true;
...
bool OPUInstrInfo::hasVALU32BitEncoding(unsigned Opcode) const {
  int Op32 = OPU::getVOPe32(Opcode);
  if (Op32 == -1)
    return false;
...
bool OPUInstrInfo::hasModifiers(unsigned Opcode) const {
  // The src0_modifier operand is present on all instructions
  // that have modifiers.

  return OPU::getNamedOperandIdx(Opcode,
                                    OPU::OpName::src0_modifiers) != -1;
}

bool OPUInstrInfo::hasModifiersSet(const MachineInstr &MI,
                                  unsigned OpName) const {
  const MachineOperand *Mods = getNamedOperand(MI, OpName);
  return Mods && Mods->getImm();
}

bool OPUInstrInfo::hasAnyModifiersSet(const MachineInstr &MI) const {
  return hasModifiersSet(MI, OPU::OpName::src0_modifiers) ||
         hasModifiersSet(MI, OPU::OpName::src1_modifiers) ||
         hasModifiersSet(MI, OPU::OpName::src2_modifiers) ||
         hasModifiersSet(MI, OPU::OpName::clamp) ||
         hasModifiersSet(MI, OPU::OpName::omod);
}

bool OPUInstrInfo::canShrink(const MachineInstr &MI,
  ...
  if (Src2) {
    switch (MI.getOpcode()) {
      ...
      case OPU::V_ADDC_U32_e64:
      case OPU::V_SUBB_U32_e64:
      case OPU::V_SUBBREV_U32_e64: {
        const MachineOperand *Src1
          = getNamedOperand(MI, OPU::OpName::src1);
  ...
  if (hasModifiersSet(MI, OPU::OpName::src0_modifiers))
    return false;
  ...
  return !hasModifiersSet(MI, OPU::OpName::omod) &&
         !hasModifiersSet(MI, OPU::OpName::clamp);
}
...
static void copyFlagsToImplicitVCC(MachineInstr &MI,
  ...
  for (MachineOperand &Use : MI.implicit_operands()) {
    if (Use.isUse() && Use.getReg() == OPU::VCC) {
...
MachineInstr *OPUInstrInfo::buildShrunkInst(MachineInstr &MI,
                                           unsigned Op32) const {
  MachineBasicBlock *MBB = MI.getParent();;
  ...
  int Op32DstIdx = OPU::getNamedOperandIdx(Op32, OPU::OpName::vdst);
  if (Op32DstIdx != -1) {
    // dst
    Inst32.add(MI.getOperand(0));
  } else {
    assert(((MI.getOperand(0).getReg() == OPU::VCC) ||
            (MI.getOperand(0).getReg() == OPU::VCC_LO)) &&
           "Unexpected case");
  }

  Inst32.add(*getNamedOperand(MI, OPU::OpName::src0));
  ...
  if (Src2) {
    int Op32Src2Idx = OPU::getNamedOperandIdx(Op32, OPU::OpName::src2);
    if (Op32Src2Idx != -1) {
      Inst32.add(*Src2);
    } else {
...
bool OPUInstrInfo::usesConstantBus(const MachineRegisterInfo &MRI,
  ...
  // Null is free
  if (MO.getReg() == OPU::SGPR_NULL)
    return false;

  // SGPRs use the constant bus
  if (MO.isImplicit()) {
    return MO.getReg() == OPU::M0 ||
           MO.getReg() == OPU::VCC ||
           MO.getReg() == OPU::VCC_LO;
  } else {
    return OPU::SReg_32RegClass.contains(MO.getReg()) ||
           OPU::SReg_64RegClass.contains(MO.getReg());
  }
}
...
static bool shouldReadExec(const MachineInstr &MI) {
  if (OPUInstrInfo::isVALU(MI)) {
    switch (MI.getOpcode()) {
    case OPU::V_READLANE_B32:
    case OPU::V_READLANE_B32_gfx6_gfx7:
    case OPU::V_READLANE_B32_gfx10:
    case OPU::V_READLANE_B32_vi:
    case OPU::V_WRITELANE_B32:
    case OPU::V_WRITELANE_B32_gfx6_gfx7:
    case OPU::V_WRITELANE_B32_gfx10:
    case OPU::V_WRITELANE_B32_vi:
      return false;
    }
  ...
  if (OPUInstrInfo::isGenericOpcode(MI.getOpcode()) ||
      OPUInstrInfo::isSALU(MI) ||
      OPUInstrInfo::isSMRD(MI))
    return false;

  return true;
}

static bool isSubRegOf(const OPURegisterInfo &TRI,
                       const MachineOperand &SuperVec,
                       const MachineOperand &SubReg) {
  if (TargetRegisterInfo::isPhysicalRegister(SubReg.getReg()))
    return TRI.isSubRegister(SuperVec.getReg(), SubReg.getReg());

  return SubReg.getSubReg() != OPU::NoSubRegister &&
         SubReg.getReg() == SuperVec.getReg();
}

bool OPUInstrInfo::verifyInstruction(const MachineInstr &MI,
  ...
  int Src0Idx = OPU::getNamedOperandIdx(Opcode, OPU::OpName::src0);
  int Src1Idx = OPU::getNamedOperandIdx(Opcode, OPU::OpName::src1);
  int Src2Idx = OPU::getNamedOperandIdx(Opcode, OPU::OpName::src2);
  ...
  if (isSDWA(MI)) {
    ...
    int DstIdx = OPU::getNamedOperandIdx(Opcode, OPU::OpName::vdst);
    ...
    const MachineOperand *DstUnused = getNamedOperand(MI, OPU::OpName::dst_unused);
    if (DstUnused && DstUnused->isImm() &&
        DstUnused->getImm() == OPU::SDWA::UNUSED_PRESERVE) {
      const MachineOperand &Dst = MI.getOperand(DstIdx);
  ...
  if (isMIMG(MI.getOpcode()) && !MI.mayStore()) {
    ...
    const MachineOperand *DMask = getNamedOperand(MI, OPU::OpName::dmask);
    if (DMask) {
      uint64_t DMaskImm = DMask->getImm();
      uint32_t RegCount =
          isGather4(MI.getOpcode()) ? 4 : countPopulation(DMaskImm);
      const MachineOperand *TFE = getNamedOperand(MI, OPU::OpName::tfe);
      const MachineOperand *LWE = getNamedOperand(MI, OPU::OpName::lwe);
      const MachineOperand *D16 = getNamedOperand(MI, OPU::OpName::d16);
      ...
      const uint32_t DstIdx =
          OPU::getNamedOperandIdx(MI.getOpcode(), OPU::OpName::vdata);
      const MachineOperand &Dst = MI.getOperand(DstIdx);
      if (Dst.isReg()) {
  ...
  if (Desc.getOpcode() != OPU::V_WRITELANE_B32
  ...
  if (Desc.getOpcode() == OPU::V_DIV_SCALE_F32 ||
      Desc.getOpcode() == OPU::V_DIV_SCALE_F64) {
    const MachineOperand &Src0 = MI.getOperand(Src0Idx);
  ...
  if (isSOPK(MI)) {
    auto Op = getNamedOperand(MI, OPU::OpName::simm16);
    if (Desc.isBranch()) {
      if (!Op->isMBB()) {
  ...
  if (Desc.getOpcode() == OPU::V_MOVRELS_B32_e32 ||
      Desc.getOpcode() == OPU::V_MOVRELS_B32_e64 ||
      Desc.getOpcode() == OPU::V_MOVRELD_B32_e32 ||
      Desc.getOpcode() == OPU::V_MOVRELD_B32_e64) {
    const bool IsDst = Desc.getOpcode() == OPU::V_MOVRELD_B32_e32 ||
                       Desc.getOpcode() == OPU::V_MOVRELD_B32_e64;
  ...
  if (shouldReadExec(MI)) {
    if (!MI.hasRegisterImplicitUseOperand(OPU::EXEC)) {
  ...
  if (isSMRD(MI)) {
    if (MI.mayStore()) {
      ...
      // soffset register.
      const MachineOperand *Soff = getNamedOperand(MI, OPU::OpName::soff);
      if (Soff && Soff->getReg() != OPU::M0) {
  ...
  if (isFLAT(MI) && !MF->getSubtarget<GCNSubtarget>().hasFlatInstOffsets()) {
    const MachineOperand *Offset = getNamedOperand(MI, OPU::OpName::offset);
    if (Offset->getImm() != 0) {
  ...
  if (isMIMG(MI)) {
    const MachineOperand *DimOp = getNamedOperand(MI, OPU::OpName::dim);
    if (DimOp) {
      int VAddr0Idx = OPU::getNamedOperandIdx(Opcode,
                                                 OPU::OpName::vaddr0);
      int SRsrcIdx = OPU::getNamedOperandIdx(Opcode, OPU::OpName::srsrc);
      const OPU::MIMGInfo *Info = OPU::getMIMGInfo(Opcode);
      const OPU::MIMGBaseOpcodeInfo *BaseOpcode =
          OPU::getMIMGBaseOpcodeInfo(Info->BaseOpcode);
      const OPU::MIMGDimInfo *Dim =
          OPU::getMIMGDimInfoByEncoding(DimOp->getImm());
  ...
  const MachineOperand *DppCt = getNamedOperand(MI, OPU::OpName::dpp_ctrl);
  if (DppCt) {
    ...
    if (DC >= DppCtrl::WAVE_SHL1 && DC <= DppCtrl::WAVE_ROR1 &&
        ST.getGeneration() >= OPUSubtarget::GFX10) {
      ErrInfo = "Invalid dpp_ctrl value: "
...
unsigned OPUInstrInfo::getVALUOp(const MachineInstr &MI) const {
  switch (MI.getOpcode()) {
  default: return OPU::INSTRUCTION_LIST_END;
  case OPU::REG_SEQUENCE: return OPU::REG_SEQUENCE;
  case OPU::COPY: return OPU::COPY;
  case OPU::PHI: return OPU::PHI;
  case OPU::INSERT_SUBREG: return OPU::INSERT_SUBREG;
  case OPU::WQM: return OPU::WQM;
  case OPU::WWM: return OPU::WWM;
  case OPU::S_MOV_B32: {
    const MachineRegisterInfo &MRI = MI.getParent()->getParent()->getRegInfo();
    return MI.getOperand(1).isReg() ||
           RI.isAGPR(MRI, MI.getOperand(0).getReg()) ?
           OPU::COPY : OPU::V_MOV_B32_e32;
  }
  case OPU::S_ADD_I32:
    return ST.hasAddNoCarry() ? OPU::V_ADD_U32_e64 : OPU::V_ADD_I32_e32;
  case OPU::S_ADDC_U32:
    return OPU::V_ADDC_U32_e32;
  case OPU::S_SUB_I32:
    return ST.hasAddNoCarry() ? OPU::V_SUB_U32_e64 : OPU::V_SUB_I32_e32;
    // FIXME: These are not consistently handled, and selected when the carry is
    // used.
  case OPU::S_ADD_U32:
    return OPU::V_ADD_I32_e32;
  case OPU::S_SUB_U32:
    return OPU::V_SUB_I32_e32;
  case OPU::S_SUBB_U32: return OPU::V_SUBB_U32_e32;
  case OPU::S_MUL_I32: return OPU::V_MUL_LO_U32;
  case OPU::S_MUL_HI_U32: return OPU::V_MUL_HI_U32;
  case OPU::S_MUL_HI_I32: return OPU::V_MUL_HI_I32;
  case OPU::S_AND_B32: return OPU::V_AND_B32_e64;
  case OPU::S_OR_B32: return OPU::V_OR_B32_e64;
  case OPU::S_XOR_B32: return OPU::V_XOR_B32_e64;
  case OPU::S_XNOR_B32:
    return ST.hasDLInsts() ? OPU::V_XNOR_B32_e64 : OPU::INSTRUCTION_LIST_END;
  case OPU::S_MIN_I32: return OPU::V_MIN_I32_e64;
  case OPU::S_MIN_U32: return OPU::V_MIN_U32_e64;
  case OPU::S_MAX_I32: return OPU::V_MAX_I32_e64;
  case OPU::S_MAX_U32: return OPU::V_MAX_U32_e64;
  case OPU::S_ASHR_I32: return OPU::V_ASHR_I32_e32;
  case OPU::S_ASHR_I64: return OPU::V_ASHR_I64;
  case OPU::S_LSHL_B32: return OPU::V_LSHL_B32_e32;
  case OPU::S_LSHL_B64: return OPU::V_LSHL_B64;
  case OPU::S_LSHR_B32: return OPU::V_LSHR_B32_e32;
  case OPU::S_LSHR_B64: return OPU::V_LSHR_B64;
  case OPU::S_SEXT_I32_I8: return OPU::V_BFE_I32;
  case OPU::S_SEXT_I32_I16: return OPU::V_BFE_I32;
  case OPU::S_BFE_U32: return OPU::V_BFE_U32;
  case OPU::S_BFE_I32: return OPU::V_BFE_I32;
  case OPU::S_BFM_B32: return OPU::V_BFM_B32_e64;
  case OPU::S_BREV_B32: return OPU::V_BFREV_B32_e32;
  case OPU::S_NOT_B32: return OPU::V_NOT_B32_e32;
  case OPU::S_NOT_B64: return OPU::V_NOT_B32_e32;
  case OPU::S_CMP_EQ_I32: return OPU::V_CMP_EQ_I32_e32;
  case OPU::S_CMP_LG_I32: return OPU::V_CMP_NE_I32_e32;
  case OPU::S_CMP_GT_I32: return OPU::V_CMP_GT_I32_e32;
  case OPU::S_CMP_GE_I32: return OPU::V_CMP_GE_I32_e32;
  case OPU::S_CMP_LT_I32: return OPU::V_CMP_LT_I32_e32;
  case OPU::S_CMP_LE_I32: return OPU::V_CMP_LE_I32_e32;
  case OPU::S_CMP_EQ_U32: return OPU::V_CMP_EQ_U32_e32;
  case OPU::S_CMP_LG_U32: return OPU::V_CMP_NE_U32_e32;
  case OPU::S_CMP_GT_U32: return OPU::V_CMP_GT_U32_e32;
  case OPU::S_CMP_GE_U32: return OPU::V_CMP_GE_U32_e32;
  case OPU::S_CMP_LT_U32: return OPU::V_CMP_LT_U32_e32;
  case OPU::S_CMP_LE_U32: return OPU::V_CMP_LE_U32_e32;
  case OPU::S_CMP_EQ_U64: return OPU::V_CMP_EQ_U64_e32;
  case OPU::S_CMP_LG_U64: return OPU::V_CMP_NE_U64_e32;
  case OPU::S_BCNT1_I32_B32: return OPU::V_BCNT_U32_B32_e64;
  case OPU::S_FF1_I32_B32: return OPU::V_FFBL_B32_e32;
  case OPU::S_FLBIT_I32_B32: return OPU::V_FFBH_U32_e32;
  case OPU::S_FLBIT_I32: return OPU::V_FFBH_I32_e64;
  case OPU::S_CBRANCH_SCC0: return OPU::S_CBRANCH_VCCZ;
  case OPU::S_CBRANCH_SCC1: return OPU::S_CBRANCH_VCCNZ;
  }
  llvm_unreachable(
      "Unexpected scalar opcode without corresponding vector one!");
}

const TargetRegisterClass *OPUInstrInfo::getOpRegClass(const MachineInstr &MI,
                                                      unsigned OpNo) const {
  const MachineRegisterInfo &MRI = MI.getParent()->getParent()->getRegInfo();
...
void OPUInstrInfo::legalizeOpWithMove(MachineInstr &MI, unsigned OpIdx) const {
  MachineBasicBlock::iterator I = MI;
  MachineBasicBlock *MBB = MI.getParent();
  MachineOperand &MO = MI.getOperand(OpIdx);
  MachineRegisterInfo &MRI = MBB->getParent()->getRegInfo();
  const OPURegisterInfo *TRI =
      static_cast<const OPURegisterInfo*>(MRI.getTargetRegisterInfo());
  unsigned RCID = get(MI.getOpcode()).OpInfo[OpIdx].RegClass;
  const TargetRegisterClass *RC = RI.getRegClass(RCID);
  unsigned Size = TRI->getRegSizeInBits(*RC);
  unsigned Opcode = (Size == 64) ? OPU::V_MOV_B64_PSEUDO : OPU::V_MOV_B32_e32;
  if (MO.isReg())
    Opcode = OPU::COPY;
  else if (RI.isSGPRClass(RC))
    Opcode = (Size == 64) ? OPU::S_MOV_B64 : OPU::S_MOV_B32;

  const TargetRegisterClass *VRC = RI.getEquivalentVGPRClass(RC);
  if (RI.getCommonSubClass(&OPU::VReg_64RegClass, VRC))
    VRC = &OPU::VReg_64RegClass;
  else
    VRC = &OPU::VGPR_32RegClass;
...
unsigned OPUInstrInfo::buildExtractSubReg(MachineBasicBlock::iterator MI,
  ...
  if (SuperReg.getSubReg() == OPU::NoSubRegister) {
...
MachineOperand OPUInstrInfo::buildExtractSubRegOrImm(
...
void OPUInstrInfo::swapOperands(MachineInstr &Inst) const {
...
bool OPUInstrInfo::isLegalRegOperand(const MachineRegisterInfo &MRI,
...
bool OPUInstrInfo::isLegalVSrcOperand(const MachineRegisterInfo &MRI,
                                     const MCOperandInfo &OpInfo,
                                     const MachineOperand &MO) const {
  if (MO.isReg())
...
bool OPUInstrInfo::isOperandLegal(const MachineInstr &MI, unsigned OpIdx,
                                 const MachineOperand *MO) const {
  const MachineFunction &MF = *MI.getParent()->getParent();
  const MachineRegisterInfo &MRI = MF.getRegInfo();
  const MCInstrDesc &InstDesc = MI.getDesc();
  ...
  if (isVALU(MI) && usesConstantBus(MRI, *MO, OpInfo)) {
    ...
    for (unsigned i = 0, e = MI.getNumOperands(); i != e; ++i) {
      ...
      } else if (InstDesc.OpInfo[i].OperandType == OPU::OPERAND_KIMM32) {
        if (--ConstantBusLimit <= 0)
          return false;
      } else if (isVOP3(MI) && OPU::isOPUSrcOperand(InstDesc, i) &&
...
void OPUInstrInfo::legalizeOperandsVOP2(MachineRegisterInfo &MRI,
  ...
  bool HasImplicitSGPR = findImplicitSGPRRead(MI) != OPU::NoRegister;
  ...
  if (Opc == OPU::V_WRITELANE_B32) {
    const DebugLoc &DL = MI.getDebugLoc();
    if (Src0.isReg() && RI.isVGPR(MRI, Src0.getReg())) {
      unsigned Reg = MRI.createVirtualRegister(&OPU::SReg_32_XM0RegClass);
      BuildMI(*MI.getParent(), MI, DL, get(OPU::V_READFIRSTLANE_B32), Reg)
          .add(Src0);
  ...
  // select is uniform.
  if (Opc == OPU::V_READLANE_B32 && Src1.isReg() &&
...
void OPUInstrInfo::legalizeOperandsVOP3(MachineRegisterInfo &MRI,
                                       MachineInstr &MI) const {
  unsigned Opc = MI.getOpcode();

  int VOP3Idx[3] = {
    OPU::getNamedOperandIdx(Opc, OPU::OpName::src0),
    OPU::getNamedOperandIdx(Opc, OPU::OpName::src1),
    OPU::getNamedOperandIdx(Opc, OPU::OpName::src2)
  };

  if (Opc == OPU::V_PERMLANE16_B32 ||
      Opc == OPU::V_PERMLANEX16_B32) {
...
unsigned OPUInstrInfo::readlaneVGPRToSGPR(unsigned SrcReg, MachineInstr &UseMI,
  ...
  if (SubRegs == 1) {
    BuildMI(*UseMI.getParent(), UseMI, UseMI.getDebugLoc(),
            get(OPU::V_READFIRSTLANE_B32), DstReg)
        .addReg(SrcReg);
    return DstReg;
  }
  ...
  for (unsigned i = 0; i < SubRegs; ++i) {
    unsigned SGPR = MRI.createVirtualRegister(&OPU::SGPR_32RegClass);
    BuildMI(*UseMI.getParent(), UseMI, UseMI.getDebugLoc(),
            get(OPU::V_READFIRSTLANE_B32), SGPR)
...
void OPUInstrInfo::legalizeOperandsSMRD(MachineRegisterInfo &MRI,
                                       MachineInstr &MI) const {
  ...
  MachineOperand *SBase = getNamedOperand(MI, OPU::OpName::sbase);
  if (SBase && !RI.isSGPRClass(MRI.getRegClass(SBase->getReg()))) {
    unsigned SGPR = readlaneVGPRToSGPR(SBase->getReg(), MI, MRI);
    SBase->setReg(SGPR);
  }
  MachineOperand *SOff = getNamedOperand(MI, OPU::OpName::soff);
...
void OPUInstrInfo::legalizeGenericOperand(MachineBasicBlock &InsertMBB,
                                         MachineBasicBlock::iterator I,
  ...
  MachineInstr *Copy =
      BuildMI(InsertMBB, I, DL, get(OPU::COPY), DstReg).add(Op);
...
static void
emitLoadSRsrcFromVGPRLoop(const OPUInstrInfo &TII, MachineRegisterInfo &MRI,
  ...
  unsigned SaveExecOpc =
      ST.isWave32() ? OPU::S_AND_SAVEEXEC_B32 : OPU::S_AND_SAVEEXEC_B64;
  unsigned XorTermOpc =
      ST.isWave32() ? OPU::S_XOR_B32_term : OPU::S_XOR_B64_term;
  unsigned AndOpc =
      ST.isWave32() ? OPU::S_AND_B32 : OPU::S_AND_B64;
  const auto *BoolXExecRC = TRI->getRegClass(OPU::SReg_1_XEXECRegClassID);
  ...
  unsigned SRsrcSub0 = MRI.createVirtualRegister(&OPU::SGPR_32RegClass);
  unsigned SRsrcSub1 = MRI.createVirtualRegister(&OPU::SGPR_32RegClass);
  unsigned SRsrcSub2 = MRI.createVirtualRegister(&OPU::SGPR_32RegClass);
  unsigned SRsrcSub3 = MRI.createVirtualRegister(&OPU::SGPR_32RegClass);
  unsigned SRsrc = MRI.createVirtualRegister(&OPU::SReg_128RegClass);

  // Beginning of the loop, read the next Rsrc variant.
  BuildMI(LoopBB, I, DL, TII.get(OPU::V_READFIRSTLANE_B32), SRsrcSub0)
      .addReg(VRsrc, VRsrcUndef, OPU::sub0);
  BuildMI(LoopBB, I, DL, TII.get(OPU::V_READFIRSTLANE_B32), SRsrcSub1)
      .addReg(VRsrc, VRsrcUndef, OPU::sub1);
  BuildMI(LoopBB, I, DL, TII.get(OPU::V_READFIRSTLANE_B32), SRsrcSub2)
      .addReg(VRsrc, VRsrcUndef, OPU::sub2);
  BuildMI(LoopBB, I, DL, TII.get(OPU::V_READFIRSTLANE_B32), SRsrcSub3)
      .addReg(VRsrc, VRsrcUndef, OPU::sub3);

  BuildMI(LoopBB, I, DL, TII.get(OPU::REG_SEQUENCE), SRsrc)
      .addReg(SRsrcSub0)
      .addImm(OPU::sub0)
      .addReg(SRsrcSub1)
      .addImm(OPU::sub1)
  ...
  BuildMI(LoopBB, I, DL, TII.get(OPU::V_CMP_EQ_U64_e64), CondReg0)
      .addReg(SRsrc, 0, OPU::sub0_sub1)
      .addReg(VRsrc, 0, OPU::sub0_sub1);
  BuildMI(LoopBB, I, DL, TII.get(OPU::V_CMP_EQ_U64_e64), CondReg1)
      .addReg(SRsrc, 0, OPU::sub2_sub3)
      .addReg(VRsrc, 0, OPU::sub2_sub3);
  ...
  BuildMI(LoopBB, I, DL, TII.get(XorTermOpc), Exec)
      .addReg(Exec)
      .addReg(SaveExec);
  BuildMI(LoopBB, I, DL, TII.get(OPU::S_CBRANCH_EXECNZ)).addMBB(&LoopBB);
}
...
static void loadSRsrcFromVGPR(const OPUInstrInfo &TII, MachineInstr &MI,
  ...
  const DebugLoc &DL = MI.getDebugLoc();
  unsigned Exec = ST.isWave32() ? OPU::EXEC_LO : OPU::EXEC;
  unsigned MovExecOpc = ST.isWave32() ? OPU::S_MOV_B32 : OPU::S_MOV_B64;
  const auto *BoolXExecRC = TRI->getRegClass(OPU::SReg_1_XEXECRegClassID);
...
extractRsrcPtr(const OPUInstrInfo &TII, MachineInstr &MI, MachineOperand &Rsrc) {
  MachineBasicBlock &MBB = *MI.getParent();
  MachineFunction &MF = *MBB.getParent();
  MachineRegisterInfo &MRI = MF.getRegInfo();
  ...
  unsigned RsrcPtr =
      TII.buildExtractSubReg(MI, MRI, Rsrc, &OPU::VReg_128RegClass,
                             OPU::sub0_sub1, &OPU::VReg_64RegClass);

  // Create an empty resource descriptor
  unsigned Zero64 = MRI.createVirtualRegister(&OPU::SReg_64RegClass);
  unsigned SRsrcFormatLo = MRI.createVirtualRegister(&OPU::SGPR_32RegClass);
  unsigned SRsrcFormatHi = MRI.createVirtualRegister(&OPU::SGPR_32RegClass);
  unsigned NewSRsrc = MRI.createVirtualRegister(&OPU::SReg_128RegClass);
  uint64_t RsrcDataFormat = TII.getDefaultRsrcDataFormat();

  // Zero64 = 0
  BuildMI(MBB, MI, MI.getDebugLoc(), TII.get(OPU::S_MOV_B64), Zero64)
      .addImm(0);

  // SRsrcFormatLo = RSRC_DATA_FORMAT{31-0}
  BuildMI(MBB, MI, MI.getDebugLoc(), TII.get(OPU::S_MOV_B32), SRsrcFormatLo)
      .addImm(RsrcDataFormat & 0xFFFFFFFF);

  // SRsrcFormatHi = RSRC_DATA_FORMAT{63-32}
  BuildMI(MBB, MI, MI.getDebugLoc(), TII.get(OPU::S_MOV_B32), SRsrcFormatHi)
      .addImm(RsrcDataFormat >> 32);

  // NewSRsrc = {Zero64, SRsrcFormat}
  BuildMI(MBB, MI, MI.getDebugLoc(), TII.get(OPU::REG_SEQUENCE), NewSRsrc)
      .addReg(Zero64)
      .addImm(OPU::sub0_sub1)
...
void OPUInstrInfo::legalizeOperands(MachineInstr &MI,
  ...
  if (MI.getOpcode() == OPU::REG_SEQUENCE) {
    MachineBasicBlock *MBB = MI.getParent();
  ...
  if (MI.getOpcode() == OPU::INSERT_SUBREG) {
    unsigned Dst = MI.getOperand(0).getReg();
  ...
  // Legalize OPU_INIT_M0
  if (MI.getOpcode() == OPU::OPU_INIT_M0) {
  ...
  if (isMIMG(MI) ||
      (OPU::isShader(MF.getFunction().getCallingConv()) &&
       (isMUBUF(MI) || isMTBUF(MI)))) {
    MachineOperand *SRsrc = getNamedOperand(MI, OPU::OpName::srsrc);
    if (SRsrc && !RI.isSGPRClass(MRI.getRegClass(SRsrc->getReg()))) {
      unsigned SGPR = readlaneVGPRToSGPR(SRsrc->getReg(), MI, MRI);
      SRsrc->setReg(SGPR);
    }

    MachineOperand *SSamp = getNamedOperand(MI, OPU::OpName::ssamp);
  ...
  if (RsrcIdx != -1) {
    ...
    MachineOperand *VAddr = getNamedOperand(MI, OPU::OpName::vaddr);
    if (VAddr && OPU::getIfAddr64Inst(MI.getOpcode()) != -1) {
      // This is already an ADDR64 instruction so we need to add the pointer
      // extracted from the resource descriptor to the current value of VAddr.
      unsigned NewVAddrLo = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);
      unsigned NewVAddrHi = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);
      unsigned NewVAddr = MRI.createVirtualRegister(&OPU::VReg_64RegClass);

      const auto *BoolXExecRC = RI.getRegClass(OPU::SReg_1_XEXECRegClassID);
      unsigned CondReg0 = MRI.createVirtualRegister(BoolXExecRC);
      unsigned CondReg1 = MRI.createVirtualRegister(BoolXExecRC);
      ...
      const DebugLoc &DL = MI.getDebugLoc();
      BuildMI(MBB, MI, DL, get(OPU::V_ADD_I32_e64), NewVAddrLo)
        .addDef(CondReg0)
        .addReg(RsrcPtr, 0, OPU::sub0)
        .addReg(VAddr->getReg(), 0, OPU::sub0)
        .addImm(0);

      // NewVaddrHi = RsrcPtr:sub1 + VAddr:sub1
      BuildMI(MBB, MI, DL, get(OPU::V_ADDC_U32_e64), NewVAddrHi)
        .addDef(CondReg1, RegState::Dead)
        .addReg(RsrcPtr, 0, OPU::sub1)
        .addReg(VAddr->getReg(), 0, OPU::sub1)
        .addReg(CondReg0, RegState::Kill)
        .addImm(0);

      // NewVaddr = {NewVaddrHi, NewVaddrLo}
      BuildMI(MBB, MI, MI.getDebugLoc(), get(OPU::REG_SEQUENCE), NewVAddr)
          .addReg(NewVAddrLo)
          .addImm(OPU::sub0)
          .addReg(NewVAddrHi)
          .addImm(OPU::sub1);
      ...
      unsigned NewVAddr = MRI.createVirtualRegister(&OPU::VReg_64RegClass);
      MachineOperand *VData = getNamedOperand(MI, OPU::OpName::vdata);
      MachineOperand *Offset = getNamedOperand(MI, OPU::OpName::offset);
      MachineOperand *SOffset = getNamedOperand(MI, OPU::OpName::soffset);
      unsigned Addr64Opcode = OPU::getAddr64Inst(MI.getOpcode());

      // Atomics rith return have have an additional tied operand and are
      // missing some of the special bits.
      MachineOperand *VDataIn = getNamedOperand(MI, OPU::OpName::vdata_in);
      MachineInstr *Addr64;
      ...
      if (!VDataIn) {
        ...
        if (const MachineOperand *GLC =
                getNamedOperand(MI, OPU::OpName::glc)) {
          MIB.addImm(GLC->getImm());
        }
        if (const MachineOperand *DLC =
                getNamedOperand(MI, OPU::OpName::dlc)) {
          MIB.addImm(DLC->getImm());
        }

        MIB.addImm(getNamedImmOperand(MI, OPU::OpName::slc));

        if (const MachineOperand *TFE =
                getNamedOperand(MI, OPU::OpName::tfe)) {
        ...
        Addr64 = BuildMI(MBB, MI, MI.getDebugLoc(), get(Addr64Opcode))
                     ...
                     .add(*SOffset)
                     .add(*Offset)
                     .addImm(getNamedImmOperand(MI, OPU::OpName::slc))
                     .cloneMemRefs(MI);
      }
      ...
      // NewVaddr = {NewVaddrHi, NewVaddrLo}
      BuildMI(MBB, Addr64, Addr64->getDebugLoc(), get(OPU::REG_SEQUENCE),
              NewVAddr)
          .addReg(RsrcPtr, 0, OPU::sub0)
          .addImm(OPU::sub0)
          .addReg(RsrcPtr, 0, OPU::sub1)
...
void OPUInstrInfo::moveToVALU(MachineInstr &TopInst,
                             MachineDominatorTree *MDT) const {
  ...
  while (!Worklist.empty()) {
    ...
    default:
      break;
    case OPU::S_ADD_U64_PSEUDO:
    case OPU::S_SUB_U64_PSEUDO:
    ...
    case OPU::S_SUB_I32:
      ...
      break;
    case OPU::S_AND_B64:
      splitScalar64BitBinaryOp(Worklist, Inst, OPU::S_AND_B32, MDT);
      Inst.eraseFromParent();
      continue;

    case OPU::S_OR_B64:
      splitScalar64BitBinaryOp(Worklist, Inst, OPU::S_OR_B32, MDT);
      Inst.eraseFromParent();
      continue;

    case OPU::S_XOR_B64:
      splitScalar64BitBinaryOp(Worklist, Inst, OPU::S_XOR_B32, MDT);
      Inst.eraseFromParent();
      continue;

    case OPU::S_NAND_B64:
      splitScalar64BitBinaryOp(Worklist, Inst, OPU::S_NAND_B32, MDT);
      Inst.eraseFromParent();
      continue;

    case OPU::S_NOR_B64:
      splitScalar64BitBinaryOp(Worklist, Inst, OPU::S_NOR_B32, MDT);
      Inst.eraseFromParent();
      continue;

    case OPU::S_XNOR_B64:
      if (ST.hasDLInsts())
        splitScalar64BitBinaryOp(Worklist, Inst, OPU::S_XNOR_B32, MDT);
      else
        splitScalar64BitXnor(Worklist, Inst, MDT);
      Inst.eraseFromParent();
      continue;

    case OPU::S_ANDN2_B64:
      splitScalar64BitBinaryOp(Worklist, Inst, OPU::S_ANDN2_B32, MDT);
      Inst.eraseFromParent();
      continue;

    case OPU::S_ORN2_B64:
      splitScalar64BitBinaryOp(Worklist, Inst, OPU::S_ORN2_B32, MDT);
      Inst.eraseFromParent();
      continue;

    case OPU::S_NOT_B64:
      splitScalar64BitUnaryOp(Worklist, Inst, OPU::S_NOT_B32);
      Inst.eraseFromParent();
      continue;

    case OPU::S_BCNT1_I32_B64:
    ...
    case OPU::S_LSHL_B32:
      if (ST.hasOnlyRevVALUShifts()) {
        NewOpcode = OPU::V_LSHLREV_B32_e64;
        swapOperands(Inst);
      }
      break;
    case OPU::S_ASHR_I32:
      if (ST.hasOnlyRevVALUShifts()) {
        NewOpcode = OPU::V_ASHRREV_I32_e64;
        swapOperands(Inst);
      }
      break;
    case OPU::S_LSHR_B32:
      if (ST.hasOnlyRevVALUShifts()) {
        NewOpcode = OPU::V_LSHRREV_B32_e64;
        swapOperands(Inst);
      }
      break;
    case OPU::S_LSHL_B64:
      if (ST.hasOnlyRevVALUShifts()) {
        NewOpcode = OPU::V_LSHLREV_B64;
        swapOperands(Inst);
      }
      break;
    case OPU::S_ASHR_I64:
      if (ST.hasOnlyRevVALUShifts()) {
        NewOpcode = OPU::V_ASHRREV_I64;
        swapOperands(Inst);
      }
      break;
    case OPU::S_LSHR_B64:
      if (ST.hasOnlyRevVALUShifts()) {
        NewOpcode = OPU::V_LSHRREV_B64;
        swapOperands(Inst);
      }
      break;
    ...
    case OPU::S_CBRANCH_SCC1:
      ...
      if (ST.isWave32())
        BuildMI(*MBB, Inst, Inst.getDebugLoc(), get(OPU::S_AND_B32),
                OPU::VCC_LO)
            .addReg(OPU::EXEC_LO)
            .addReg(OPU::VCC_LO);
      else
        BuildMI(*MBB, Inst, Inst.getDebugLoc(), get(OPU::S_AND_B64),
                OPU::VCC)
            .addReg(OPU::EXEC)
            .addReg(OPU::VCC);
      break;
    ...
    case OPU::S_PACK_LL_B32_B16:
    case OPU::S_PACK_LH_B32_B16:
    case OPU::S_PACK_HH_B32_B16:
    ...
    case OPU::S_NAND_B32:
      splitScalarNotBinop(Worklist, Inst, OPU::S_AND_B32);
      Inst.eraseFromParent();
      continue;

    case OPU::S_NOR_B32:
      splitScalarNotBinop(Worklist, Inst, OPU::S_OR_B32);
      Inst.eraseFromParent();
      continue;

    case OPU::S_ANDN2_B32:
      splitScalarBinOpN2(Worklist, Inst, OPU::S_AND_B32);
      Inst.eraseFromParent();
      continue;

    case OPU::S_ORN2_B32:
      splitScalarBinOpN2(Worklist, Inst, OPU::S_OR_B32);
      Inst.eraseFromParent();
      continue;
    }

    if (NewOpcode == OPU::INSTRUCTION_LIST_END) {
    ...
    if (Opcode == OPU::S_SEXT_I32_I8 || Opcode == OPU::S_SEXT_I32_I16) {
      // We are converting these to a BFE, so we need to add the missing
      // operands for the size and offset.
      unsigned Size = (Opcode == OPU::S_SEXT_I32_I8) ? 8 : 16;
      Inst.addOperand(MachineOperand::CreateImm(0));
      Inst.addOperand(MachineOperand::CreateImm(Size));

    } else if (Opcode == OPU::S_BCNT1_I32_B32) {
    ...
    if (Opcode == OPU::S_BFE_I32 || Opcode == OPU::S_BFE_U32) {
    ...
    unsigned NewDstReg = OPU::NoRegister;
    if (HasDst) {
      ...
      if (Inst.isCopy() &&
        ...
        for (unsigned I = Inst.getNumOperands() - 1; I != 0; --I)
          Inst.RemoveOperand(I);
        Inst.setDesc(get(OPU::IMPLICIT_DEF));
        continue;
      }
...
bool OPUInstrInfo::moveScalarAddSub(SetVectorType &Worklist, MachineInstr &Inst,
                                   MachineDominatorTree *MDT) const {
  if (ST.hasAddNoCarry()) {
    ...
    unsigned ResultReg = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);

    unsigned Opc = Inst.getOpcode();
    assert(Opc == OPU::S_ADD_I32 || Opc == OPU::S_SUB_I32);

    unsigned NewOpc = Opc == OPU::S_ADD_I32 ?
      OPU::V_ADD_U32_e64 : OPU::V_SUB_U32_e64;

    assert(Inst.getOperand(3).getReg() == OPU::SCC);
...
void OPUInstrInfo::lowerScalarAbs(SetVectorType &Worklist,
  ...
  unsigned TmpReg = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);
  unsigned ResultReg = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);

  unsigned SubOp = ST.hasAddNoCarry() ?
    OPU::V_SUB_U32_e32 : OPU::V_SUB_I32_e32;

  BuildMI(MBB, MII, DL, get(SubOp), TmpReg)
    .addImm(0)
    .addReg(Src.getReg());

  BuildMI(MBB, MII, DL, get(OPU::V_MAX_I32_e64), ResultReg)
    .addReg(Src.getReg())
    .addReg(TmpReg);
...
void OPUInstrInfo::lowerScalarXnor(SetVectorType &Worklist,
                                  MachineInstr &Inst) const {
  MachineBasicBlock &MBB = *Inst.getParent();
  MachineRegisterInfo &MRI = MBB.getParent()->getRegInfo();
  MachineBasicBlock::iterator MII = Inst;
  ...
  if (ST.hasDLInsts()) {
    unsigned NewDest = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);
    legalizeGenericOperand(MBB, MII, &OPU::VGPR_32RegClass, Src0, MRI, DL);
    legalizeGenericOperand(MBB, MII, &OPU::VGPR_32RegClass, Src1, MRI, DL);

    BuildMI(MBB, MII, DL, get(OPU::V_XNOR_B32_e64), NewDest)
      .add(Src0)
      .add(Src1);
    ...
    bool Src1IsSGPR = Src1.isReg() &&
                      RI.isSGPRClass(MRI.getRegClass(Src1.getReg()));
    MachineInstr *Xor;
    unsigned Temp = MRI.createVirtualRegister(&OPU::SReg_32_XM0RegClass);
    unsigned NewDest = MRI.createVirtualRegister(&OPU::SReg_32_XM0RegClass);
    ...
    // unit as necessary.
    if (Src0IsSGPR) {
      BuildMI(MBB, MII, DL, get(OPU::S_NOT_B32), Temp).add(Src0);
      Xor = BuildMI(MBB, MII, DL, get(OPU::S_XOR_B32), NewDest)
      .addReg(Temp)
      .add(Src1);
    } else if (Src1IsSGPR) {
      BuildMI(MBB, MII, DL, get(OPU::S_NOT_B32), Temp).add(Src1);
      Xor = BuildMI(MBB, MII, DL, get(OPU::S_XOR_B32), NewDest)
      .add(Src0)
      .addReg(Temp);
    } else {
      Xor = BuildMI(MBB, MII, DL, get(OPU::S_XOR_B32), Temp)
        .add(Src0)
        .add(Src1);
      MachineInstr *Not =
          BuildMI(MBB, MII, DL, get(OPU::S_NOT_B32), NewDest).addReg(Temp);
      Worklist.insert(Not);
    }
...
void OPUInstrInfo::splitScalarNotBinop(SetVectorType &Worklist,
                                      MachineInstr &Inst,
  ...
  unsigned NewDest = MRI.createVirtualRegister(&OPU::SReg_32_XM0RegClass);
  unsigned Interm = MRI.createVirtualRegister(&OPU::SReg_32_XM0RegClass);

  MachineInstr &Op = *BuildMI(MBB, MII, DL, get(Opcode), Interm)
    .add(Src0)
    .add(Src1);

  MachineInstr &Not = *BuildMI(MBB, MII, DL, get(OPU::S_NOT_B32), NewDest)
    .addReg(Interm);
...
void OPUInstrInfo::splitScalarBinOpN2(SetVectorType& Worklist,
                                     MachineInstr &Inst,
                                     unsigned Opcode) const {
  MachineBasicBlock &MBB = *Inst.getParent();
  MachineRegisterInfo &MRI = MBB.getParent()->getRegInfo();
  ...
  MachineInstr &Not = *BuildMI(MBB, MII, DL, get(OPU::S_NOT_B32), Interm)
    .add(Src1);
...
void OPUInstrInfo::splitScalar64BitUnaryOp(
  ...
  const TargetRegisterClass *Src0SubRC = RI.getSubRegClass(Src0RC, OPU::sub0);
  ...
  const TargetRegisterClass *NewDestSubRC = RI.getSubRegClass(NewDestRC, OPU::sub0);
...
void OPUInstrInfo::splitScalar64BitAddSub(SetVectorType &Worklist,
                                         MachineInstr &Inst,
                                         MachineDominatorTree *MDT) const {
  bool IsAdd = (Inst.getOpcode() == OPU::S_ADD_U64_PSEUDO);

  MachineBasicBlock &MBB = *Inst.getParent();
  MachineRegisterInfo &MRI = MBB.getParent()->getRegInfo();
  const auto *CarryRC = RI.getRegClass(OPU::SReg_1_XEXECRegClassID);

  unsigned FullDestReg = MRI.createVirtualRegister(&OPU::VReg_64RegClass);
  unsigned DestSub0 = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);
  unsigned DestSub1 = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);
  ...
  const TargetRegisterClass *Src0SubRC = RI.getSubRegClass(Src0RC, OPU::sub0);
  const TargetRegisterClass *Src1SubRC = RI.getSubRegClass(Src1RC, OPU::sub0);
  ...
  unsigned LoOpc = IsAdd ? OPU::V_ADD_I32_e64 : OPU::V_SUB_I32_e64;
  MachineInstr *LoHalf =
  ...
  unsigned HiOpc = IsAdd ? OPU::V_ADDC_U32_e64 : OPU::V_SUBB_U32_e64;
  MachineInstr *HiHalf =
...
void OPUInstrInfo::splitScalar64BitBinaryOp(SetVectorType &Worklist,
...
void OPUInstrInfo::splitScalar64BitXnor(SetVectorType &Worklist,
                                       MachineInstr &Inst,
                                       MachineDominatorTree *MDT) const {
  MachineBasicBlock &MBB = *Inst.getParent();
  MachineRegisterInfo &MRI = MBB.getParent()->getRegInfo();
  ...
  unsigned Interm = MRI.createVirtualRegister(&OPU::SReg_64RegClass);
  ...
  BuildMI(MBB, MII, DL, get(OPU::S_NOT_B64), Interm)
    .add(*Op0);

  unsigned NewDest = MRI.createVirtualRegister(DestRC);

  MachineInstr &Xor = *BuildMI(MBB, MII, DL, get(OPU::S_XOR_B64), NewDest)
    .addReg(Interm)
    .add(*Op1);
...
void OPUInstrInfo::splitScalar64BitBCNT(
  ...
  const MCInstrDesc &InstDesc = get(OPU::V_BCNT_U32_B32_e64);
  const TargetRegisterClass *SrcRC = Src.isReg() ?
    MRI.getRegClass(Src.getReg()) :
    &OPU::SGPR_32RegClass;

  unsigned MidReg = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);
  unsigned ResultReg = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);

  const TargetRegisterClass *SrcSubRC = RI.getSubRegClass(SrcRC, OPU::sub0);
...
void OPUInstrInfo::splitScalar64BitBFE(SetVectorType &Worklist,
                                      MachineInstr &Inst) const {
  MachineBasicBlock &MBB = *Inst.getParent();
  MachineRegisterInfo &MRI = MBB.getParent()->getRegInfo();
  MachineBasicBlock::iterator MII = Inst;
  ...
  assert(Inst.getOpcode() == OPU::S_BFE_I64 && BitWidth <= 32 &&
         Offset == 0 && "Not implemented");

  if (BitWidth < 32) {
    unsigned MidRegLo = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);
    unsigned MidRegHi = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);
    unsigned ResultReg = MRI.createVirtualRegister(&OPU::VReg_64RegClass);

    BuildMI(MBB, MII, DL, get(OPU::V_BFE_I32), MidRegLo)
        .addReg(Inst.getOperand(1).getReg(), 0, OPU::sub0)
        .addImm(0)
        .addImm(BitWidth);

    BuildMI(MBB, MII, DL, get(OPU::V_ASHRREV_I32_e32), MidRegHi)
      .addImm(31)
      .addReg(MidRegLo);
  ...
  BuildMI(MBB, MII, DL, get(OPU::V_ASHRREV_I32_e64), TmpReg)
    .addImm(31)
    .addReg(Src.getReg(), 0, OPU::sub0);
...
void OPUInstrInfo::addUsersToMoveToVALUWorklist(
  unsigned DstReg,
  MachineRegisterInfo &MRI,
...
void OPUInstrInfo::movePackToVALU(SetVectorType &Worklist,
                                 MachineRegisterInfo &MRI,
                                 MachineInstr &Inst) const {
  unsigned ResultReg = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);
  MachineBasicBlock *MBB = Inst.getParent();
  ...
  case OPU::S_PACK_LL_B32_B16: {
    unsigned ImmReg = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);
    unsigned TmpReg = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);

    // FIXME: Can do a lot better if we know the high bits of src0 or src1 are
    // 0.
    BuildMI(*MBB, Inst, DL, get(OPU::V_MOV_B32_e32), ImmReg)
      .addImm(0xffff);

    BuildMI(*MBB, Inst, DL, get(OPU::V_AND_B32_e64), TmpReg)
      .addReg(ImmReg, RegState::Kill)
      .add(Src0);

    BuildMI(*MBB, Inst, DL, get(OPU::V_LSHL_OR_B32), ResultReg)
      .add(Src1)
      .addImm(16)
  ...
  case OPU::S_PACK_LH_B32_B16: {
    ...
    BuildMI(*MBB, Inst, DL, get(OPU::V_MOV_B32_e32), ImmReg)
      .addImm(0xffff);
    BuildMI(*MBB, Inst, DL, get(OPU::V_BFI_B32), ResultReg)
      .addReg(ImmReg, RegState::Kill)
      .add(Src0)
      .add(Src1);
    break;
  ...
  case OPU::S_PACK_HH_B32_B16: {
    unsigned ImmReg = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);
    unsigned TmpReg = MRI.createVirtualRegister(&OPU::VGPR_32RegClass);
    BuildMI(*MBB, Inst, DL, get(OPU::V_LSHRREV_B32_e64), TmpReg)
      .addImm(16)
      .add(Src0);
    BuildMI(*MBB, Inst, DL, get(OPU::V_MOV_B32_e32), ImmReg)
      .addImm(0xffff0000);
    BuildMI(*MBB, Inst, DL, get(OPU::V_AND_OR_B32), ResultReg)
      .add(Src1)
      .addReg(ImmReg, RegState::Kill)
      .addReg(TmpReg, RegState::Kill);
    break;
...
void OPUInstrInfo::addSCCDefUsersToVALUWorklist(MachineOperand &Op,
                                               MachineInstr &SCCDefInst,
                                               SetVectorType &Worklist) const {
  // Ensure that def inst defines SCC, which is still live.
  assert(Op.isReg() && Op.getReg() == OPU::SCC && Op.isDef() &&
  ...
  for (MachineInstr &MI : // Skip the def inst itself.
    ...
    if (MI.findRegisterUseOperandIdx(OPU::SCC, false, &RI) != -1)
      Worklist.insert(&MI);
    // Exit if we find another SCC def.
    if (MI.findRegisterDefOperandIdx(OPU::SCC, false, false, &RI) != -1)
      return;
  }
}

const TargetRegisterClass *OPUInstrInfo::getDestEquivalentVGPRClass(
...
// Find the one SGPR operand we are allowed to use.
unsigned OPUInstrInfo::findUsedSGPR(const MachineInstr &MI,
  ...
  unsigned UsedSGPRs[3] = { OPU::NoRegister };
  const MachineRegisterInfo &MRI = MI.getParent()->getParent()->getRegInfo();
  ...
  if (UsedSGPRs[0] != OPU::NoRegister) {
    if (UsedSGPRs[0] == UsedSGPRs[1] || UsedSGPRs[0] == UsedSGPRs[2])
      SGPRReg = UsedSGPRs[0];
  }

  if (SGPRReg == OPU::NoRegister && UsedSGPRs[1] != OPU::NoRegister) {
...
MachineOperand *OPUInstrInfo::getNamedOperand(MachineInstr &MI,
                                             unsigned OperandName) const {
  int Idx = OPU::getNamedOperandIdx(MI.getOpcode(), OperandName);
  if (Idx == -1)
    return nullptr;
...
uint64_t OPUInstrInfo::getDefaultRsrcDataFormat() const {
  if (ST.getGeneration() >= OPUSubtarget::GFX10) {
    return (22ULL << 44) | // IMG_FORMAT_32_FLOAT
           (1ULL << 56) | // RESOURCE_LEVEL = 1
           (3ULL << 60); // OOB_SELECT = 3
  }

  uint64_t RsrcDataFormat = OPU::RSRC_DATA_FORMAT;
  if (ST.isAmdHsaOS()) {
    // Set ATC = 1. GFX9 doesn't have this bit.
    if (ST.getGeneration() <= OPUSubtarget::VOLCANIC_ISLANDS)
...
uint64_t OPUInstrInfo::getScratchRsrcWords23() const {
  uint64_t Rsrc23 = getDefaultRsrcDataFormat() |
                    OPU::RSRC_TID_ENABLE |
                    0xffffffff; // Size;

  // GFX9 doesn't have ELEMENT_OPUZE.
  if (ST.getGeneration() <= OPUSubtarget::VOLCANIC_ISLANDS) {
    uint64_t EltSizeValue = Log2_32(ST.getMaxPrivateElementSize()) - 1;
    Rsrc23 |= EltSizeValue << OPU::RSRC_ELEMENT_OPUZE_SHIFT;
  }

  // IndexStride = 64 / 32.
  uint64_t IndexStride = ST.getWavefrontSize() == 64 ? 3 : 2;
  Rsrc23 |= IndexStride << OPU::RSRC_INDEX_STRIDE_SHIFT;

  // If TID_ENABLE is set, DATA_FORMAT specifies stride bits [14:17].
  // Clear them unless we want a huge stride.
  if (ST.getGeneration() >= OPUSubtarget::VOLCANIC_ISLANDS &&
      ST.getGeneration() <= OPUSubtarget::GFX9)
    Rsrc23 &= ~OPU::RSRC_DATA_FORMAT;

  return Rsrc23;
}

bool OPUInstrInfo::isLowLatencyInstruction(const MachineInstr &MI) const {
  unsigned Opc = MI.getOpcode();

  return isSMRD(Opc);
}

bool OPUInstrInfo::isHighLatencyInstruction(const MachineInstr &MI) const {
  unsigned Opc = MI.getOpcode();

  return isMUBUF(Opc) || isMTBUF(Opc) || isMIMG(Opc);
}

unsigned OPUInstrInfo::isStackAccess(const MachineInstr &MI,
                                    int &FrameIndex) const {
  const MachineOperand *Addr = getNamedOperand(MI, OPU::OpName::vaddr);
  if (!Addr || !Addr->isFI())
    return OPU::NoRegister;

  assert(!MI.memoperands_empty() &&
         (*MI.memoperands_begin())->getAddrSpace() == OPUAS::PRIVATE_ADDRESS);

  FrameIndex = Addr->getIndex();
  return getNamedOperand(MI, OPU::OpName::vdata)->getReg();
}

unsigned OPUInstrInfo::isSGPRStackAccess(const MachineInstr &MI,
                                        int &FrameIndex) const {
  const MachineOperand *Addr = getNamedOperand(MI, OPU::OpName::addr);
  assert(Addr && Addr->isFI());
  FrameIndex = Addr->getIndex();
  return getNamedOperand(MI, OPU::OpName::data)->getReg();
}

unsigned OPUInstrInfo::isLoadFromStackSlot(const MachineInstr &MI,
                                          int &FrameIndex) const {
  if (!MI.mayLoad())
    return OPU::NoRegister;
...
unsigned OPUInstrInfo::isStoreToStackSlot(const MachineInstr &MI,
                                         int &FrameIndex) const {
  if (!MI.mayStore())
    return OPU::NoRegister;
...
unsigned OPUInstrInfo::getInstBundleSize(const MachineInstr &MI) const {
  unsigned Size = 0;
...
unsigned OPUInstrInfo::getInstSizeInBytes(const MachineInstr &MI) const {
  unsigned Opc = MI.getOpcode();
  ...
  if (isMIMG(MI)) {
    ...
    int RSrcIdx = OPU::getNamedOperandIdx(Opc, OPU::OpName::srsrc);
...
bool OPUInstrInfo::mayAccessFlatAddressSpace(const MachineInstr &MI) const {
  if (!isFLAT(MI))
    return false;
  ...
  for (const MachineMemOperand *MMO : MI.memoperands()) {
    if (MMO->getAddrSpace() == OPUAS::FLAT_ADDRESS)
      return true;
  }
  return false;
}

bool OPUInstrInfo::isNonUniformBranchInstr(MachineInstr &Branch) const {
  return Branch.getOpcode() == OPU::OPU_NON_UNIFORM_BRCOND_PSEUDO;
}

void OPUInstrInfo::convertNonUniformIfRegion(MachineBasicBlock *IfEntry,
  ...
  if (Branch->getOpcode() == OPU::OPU_NON_UNIFORM_BRCOND_PSEUDO) {
    unsigned DstReg = MRI.createVirtualRegister(RI.getBoolRC());
    MachineInstr *OPUIF =
        BuildMI(*MF, Branch->getDebugLoc(), get(OPU::OPU_IF), DstReg)
            .add(Branch->getOperand(0))
            .add(Branch->getOperand(1));
    MachineInstr *OPUEND =
        BuildMI(*MF, Branch->getDebugLoc(), get(OPU::OPU_END_CF))
            .addReg(DstReg);

    IfEntry->erase(TI);
    IfEntry->insert(IfEntry->end(), OPUIF);
    IfEnd->insert(IfEnd->getFirstNonPHI(), OPUEND);
  }
}

void OPUInstrInfo::convertNonUniformLoopRegion(
  ...
  if (Branch->getOpcode() == OPU::OPU_NON_UNIFORM_BRCOND_PSEUDO) {
    ...
    MachineInstr *OPUIFBREAK = BuildMI(*(MF), Branch->getDebugLoc(),
                                      get(OPU::OPU_IF_BREAK), BackEdgeReg)
                                  .addReg(DstReg)
                                  .add(Branch->getOperand(0));
    MachineInstr *OPULOOP =
        BuildMI(*(MF), Branch->getDebugLoc(), get(OPU::OPU_LOOP))
            .addReg(BackEdgeReg)
            .addMBB(LoopEntry);

    LoopEntry->insert(LoopEntry->begin(), HeaderPhi);
    LoopEnd->erase(TI);
    LoopEnd->insert(LoopEnd->end(), OPUIFBREAK);
    LoopEnd->insert(LoopEnd->end(), OPULOOP);
  }
}

ArrayRef<std::pair<int, const char *>>
OPUInstrInfo::getSerializableTargetIndices() const {
  static const std::pair<int, const char *> TargetIndices[] = {
      {OPU::TI_CONSTDATA_START, "amdgpu-constdata-start"},
      {OPU::TI_SCRATCH_RSRC_DWORD0, "amdgpu-scratch-rsrc-dword0"},
      {OPU::TI_SCRATCH_RSRC_DWORD1, "amdgpu-scratch-rsrc-dword1"},
      {OPU::TI_SCRATCH_RSRC_DWORD2, "amdgpu-scratch-rsrc-dword2"},
      {OPU::TI_SCRATCH_RSRC_DWORD3, "amdgpu-scratch-rsrc-dword3"}};
...
OPUInstrInfo::CreateTargetPostRAHazardRecognizer(const InstrItineraryData *II,
...
/// pass.
ScheduleHazardRecognizer *
OPUInstrInfo::CreateTargetPostRAHazardRecognizer(const MachineFunction &MF) const {
  return new GCNHazardRecognizer(MF);
}

std::pair<unsigned, unsigned>
OPUInstrInfo::decomposeMachineOperandsTargetFlags(unsigned TF) const {
  return std::make_pair(TF & MO_MASK, TF & ~MO_MASK);
}

ArrayRef<std::pair<unsigned, const char *>>
OPUInstrInfo::getSerializableDirectMachineOperandTargetFlags() const {
...
bool OPUInstrInfo::isBasicBlockPrologue(const MachineInstr &MI) const {
  return !MI.isTerminator() && MI.getOpcode() != OPU::COPY &&
         MI.modifiesRegister(OPU::EXEC, &RI);
}

MachineInstrBuilder
OPUInstrInfo::getAddNoCarry(MachineBasicBlock &MBB,
                           MachineBasicBlock::iterator I,
                           const DebugLoc &DL,
                           unsigned DestReg) const {
  if (ST.hasAddNoCarry())
    return BuildMI(MBB, I, DL, get(OPU::V_ADD_U32_e64), DestReg);

  MachineRegisterInfo &MRI = MBB.getParent()->getRegInfo();
  unsigned UnusedCarry = MRI.createVirtualRegister(RI.getBoolRC());
  MRI.setRegAllocationHint(UnusedCarry, 0, RI.getVCC());

  return BuildMI(MBB, I, DL, get(OPU::V_ADD_I32_e64), DestReg)
           .addReg(UnusedCarry, RegState::Define | RegState::Dead);
}

bool OPUInstrInfo::isKillTerminator(unsigned Opcode) {
  switch (Opcode) {
  case OPU::OPU_KILL_F32_COND_IMM_TERMINATOR:
  case OPU::OPU_KILL_I1_TERMINATOR:
    return true;
...
const MCInstrDesc &OPUInstrInfo::getKillTerminatorFromPseudo(unsigned Opcode) const {
  switch (Opcode) {
  case OPU::OPU_KILL_F32_COND_IMM_PSEUDO:
    return get(OPU::OPU_KILL_F32_COND_IMM_TERMINATOR);
  case OPU::OPU_KILL_I1_PSEUDO:
    return get(OPU::OPU_KILL_I1_TERMINATOR);
  default:
    llvm_unreachable("invalid opcode, expected OPU_KILL_*_PSEUDO");
  }
}

void OPUInstrInfo::fixImplicitOperands(MachineInstr &MI) const {
  MachineBasicBlock *MBB = MI.getParent();
  MachineFunction *MF = MBB->getParent();
  const GCNSubtarget &ST = MF->getSubtarget<GCNSubtarget>();
  ...
  for (auto &Op : MI.implicit_operands()) {
    if (Op.isReg() && Op.getReg() == OPU::VCC)
      Op.setReg(OPU::VCC_LO);
  }
}

bool OPUInstrInfo::isBufferSMRD(const MachineInstr &MI) const {
  if (!isSMRD(MI))
    return false;

  // Check that it is using a buffer resource.
  int Idx = OPU::getNamedOperandIdx(MI.getOpcode(), OPU::OpName::sbase);
  if (Idx == -1) // e.g. s_memtime
    return false;

  const auto RCID = MI.getDesc().OpInfo[Idx].RegClass;
  return RCID == OPU::SReg_128RegClassID;
}

bool OPUInstrInfo::isLegalFLATOffset(int64_t Offset, unsigned AddrSpace,
                                    bool Signed) const {
  // TODO: Should 0 be special cased?
  if (!ST.hasFlatInstOffsets())
    return false;

  if (ST.hasFlatSegmentOffsetBug() && AddrSpace == OPUAS::FLAT_ADDRESS)
    return false;
...
// This must be kept in sync with the OPUEncodingFamily class in OPUInstrInfo.td
enum OPUEncodingFamily {
  OPU = 0,
  VI = 1,
  SDWA = 2,
  SDWA9 = 3,
...
static OPUEncodingFamily subtargetEncodingFamily(const GCNSubtarget &ST) {
  switch (ST.getGeneration()) {
  default:
    break;
  case OPUSubtarget::SOUTHERN_ISLANDS:
  case OPUSubtarget::SEA_ISLANDS:
    return OPUEncodingFamily::OPU;
  case OPUSubtarget::VOLCANIC_ISLANDS:
  case OPUSubtarget::GFX9:
    return OPUEncodingFamily::VI;
  case OPUSubtarget::GFX10:
    return OPUEncodingFamily::GFX10;
  }
  llvm_unreachable("Unknown subtarget generation!");
}

int OPUInstrInfo::pseudoToMCOpcode(int Opcode) const {
  OPUEncodingFamily Gen = subtargetEncodingFamily(ST);

  if ((get(Opcode).TSFlags & OPUInstrFlags::renamedInGFX9) != 0 &&
    ST.getGeneration() == OPUSubtarget::GFX9)
    Gen = OPUEncodingFamily::GFX9;

  // Adjust the encoding family to GFX80 for D16 buffer instructions when the
  // subtarget has UnpackedD16VMem feature.
  // TODO: remove this when we discard GFX80 encoding.
  if (ST.hasUnpackedD16VMem() && (get(Opcode).TSFlags & OPUInstrFlags::D16Buf))
    Gen = OPUEncodingFamily::GFX80;

  if (get(Opcode).TSFlags & OPUInstrFlags::SDWA) {
    switch (ST.getGeneration()) {
    default:
      Gen = OPUEncodingFamily::SDWA;
      break;
    case OPUSubtarget::GFX9:
      Gen = OPUEncodingFamily::SDWA9;
      break;
    case OPUSubtarget::GFX10:
      Gen = OPUEncodingFamily::SDWA10;
      break;
    }
  }

  int MCOp = OPU::getMCOpcode(Opcode, Gen);
...
bool llvm::execMayBeModifiedBeforeUse(const MachineRegisterInfo &MRI,
  ...
  for (auto I = std::next(DefMI.getIterator()); I != E; ++I) {
    ...
    if (I->modifiesRegister(OPU::EXEC, TRI))
      return true;
  }
