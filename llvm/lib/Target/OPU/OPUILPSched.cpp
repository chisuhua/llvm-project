...
unsigned OPUILPScheduler::getNodePriority(const SUnit *SU) const {
...
const SUnit *OPUILPScheduler::pickBest(const SUnit *left, const SUnit *right)
{
...
OPUILPScheduler::Candidate* OPUILPScheduler::pickCandidate() {
  if (AvailQueue.empty())
    return nullptr;
...
void OPUILPScheduler::releasePending() {
...
void OPUILPScheduler::advanceToCycle(unsigned NextCycle) {
...
void OPUILPScheduler::releasePredecessors(const SUnit* SU) {
...
OPUILPScheduler::schedule(ArrayRef<const SUnit*> BotRoots,
                          const ScheduleDAG &DAG) {
