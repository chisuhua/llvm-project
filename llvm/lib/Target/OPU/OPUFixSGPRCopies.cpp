//===- OPUFixSGPRCopies.cpp - Remove potential VGPR => SGPR copies ---------===//
//
...
class OPUFixSGPRCopies : public MachineFunctionPass {
  MachineDominatorTree *MDT;

public:
  static char ID;

  OPUFixSGPRCopies() : MachineFunctionPass(ID) {}

  bool runOnMachineFunction(MachineFunction &MF) override;

  StringRef getPassName() const override { return "OPU Fix SGPR copies"; }
...
INITIALIZE_PASS_BEGIN(OPUFixSGPRCopies, DEBUG_TYPE,
                     "OPU Fix SGPR copies", false, false)
INITIALIZE_PASS_DEPENDENCY(MachineDominatorTree)
INITIALIZE_PASS_END(OPUFixSGPRCopies, DEBUG_TYPE,
                     "OPU Fix SGPR copies", false, false)

char OPUFixSGPRCopies::ID = 0;

char &llvm::OPUFixSGPRCopiesID = OPUFixSGPRCopies::ID;

FunctionPass *llvm::createOPUFixSGPRCopiesPass() {
  return new OPUFixSGPRCopies();
}
...
static bool hasVectorOperands(const MachineInstr &MI,
  ...
  for (unsigned i = 0, e = MI.getNumOperands(); i != e; ++i) {
    if (!MI.getOperand(i).isReg() || !MI.getOperand(i).getReg().isVirutal())
      continue;
...
static bool isVGPRToSGPRCopy(const TargetRegisterClass *SrcRC,
                             ...
                             const OPURegisterInfo &TRI) {
  return SrcRC != &OPU::VReg_1RegClass && TRI.isSGPRClass(DstRC) &&
...
static bool isSGPRToVGPRCopy(const TargetRegisterClass *SrcRC,
                             const TargetRegisterClass *DstRC,
                             const OPURegisterInfo &TRI) {
  return DstRC != &OPU::VReg_1RegClass && TRI.isSGPRClass(SrcRC) &&
...
static bool tryChangeVGPRtoSGPRinCopy(MachineInstr &MI,
  ...
  for (const auto &MO : MRI.reg_nodbg_operands(DstReg)) {
    ...
    if (OpIdx >= UseMI->getDesc().getNumOperands() ||
        !TII->isOperandLegal(*UseMI, OpIdx, ImmOpc, &Src))
        return false;
  }
...
static bool foldVGPRCopyIntoRegSequence(MachineInstr &MI,
  ...
  for (unsigned I = 1, N = MI.getNumOperands(); I != N; I += 2) {
    ...
    BuildMI(*MI.getParent(), &MI, MI.getDebugLoc(), TII->get(OPU::COPY),
            TmpReg)
        .add(MI.getOperand(I));
/*
    if (IsAGPR) {
      ...
      unsigned Opc = NewSrcRC == &OPU::AGPR_32RegClass ?
...
static bool isSafeToFoldImmIntoCopy(const MachineInstr *Copy,
  ...
  const MachineOperand *ImmOp =
      TII->getNamedOperand(*MoveImm, OPU::OpName::src0);
  if (!ImmOp->isImm())
    return false;
  ...
  case OPU::V_MOV_B32_e32:
    SMovOp = OPU::S_MOV_B32;
    break;
  case OPU::V_MOV_B64_PSEUDO:
    SMovOp = OPU::S_MOV_B64;
    break;
  }
  Imm = ImmOp->getImm();
  return true;
...
bool OPUFixSGPRCopies::runOnMachineFunction(MachineFunction &MF) {
    ...
    for (MachineBasicBlock::iterator I = MBB.begin(), E = MBB.end();
      ...
      case OPU::WWM: {
        ...
        if (!Register::isVirtualRegister(DstReg)) {
          ...
          if (DstReg == OPU::M0 && TRI->hasVectorRegisters(SrcRC)) {
            Register TmpReg
              = MRI.createVirtualRegister(&OPU::SReg_32RegClass);

            BuildMI(MBB, MI, MI.getDebugLoc(),
                    TII->get(OPU::V_READFIRSTLANE_B32), TmpReg)
              .add(MI.getOperand(1));
  ...
  if (MF.getTarget().getOptLevel() > CodeGenOpt::None && EnableM0Merge)
    hoistAndMergeSGPRInits(OPU::M0, MRI, *MDT, TII);
