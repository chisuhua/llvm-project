//===-- OPUSchedStrategy.cpp - OPU Scheduler Strategy ---------------------===//
//
...
OPUMaxOccupancySchedStrategy::OPUMaxOccupancySchedStrategy(
    const MachineSchedContext *C)
    : GenericScheduler(C), TargetOccupancy(0), MF(nullptr) {}

void OPUMaxOccupancySchedStrategy::initialize(ScheduleDAGMI *DAG) {
  GenericScheduler::initialize(DAG);

  const OPURegisterInfo *SRI = static_cast<const OPURegisterInfo *>(TRI);
  ...
  SGPRExcessLimit =
      Context->RegClassInfo->getNumAllocatableRegs(&OPU::SPR_32RegClass) -
      ErrorMargin;
  VGPRExcessLimit =
      Context->RegClassInfo->getNumAllocatableRegs(&OPU::VPR_32RegClass) -
      ErrorMargin;
  if (TargetOccupancy) {
...
void OPUMaxOccupancySchedStrategy::initCandidate(
    SchedCandidate &Cand, SUnit *SU, bool AtTop,
    const RegPressureTracker &RPTracker, const OPURegisterInfo *SRI,
...
void OPUMaxOccupancySchedStrategy::pickNodeFromQueue(
...
SUnit *OPUMaxOccupancySchedStrategy::pickNodeBidirectional(bool &IsTopNode) {
...
SUnit *OPUMaxOccupancySchedStrategy::pickNode(bool &IsTopNode) {
...
OPUScheduleDAGMILive::OPUScheduleDAGMILive(
    MachineSchedContext *C, std::unique_ptr<MachineSchedStrategy> S)
    : ScheduleDAGMILive(C, std::move(S)), ST(MF.getSubtarget<OPUSubtarget>()),
...
void OPUScheduleDAGMILive::schedule() {
  if (Stage == 0) {
  ...
  OPURegPressure PressureBefore;
  if (LIS) {
    PressureBefore = Pressure[RegionIdx];

    LLVM_DEBUG(dbgs() << "Pressure before scheduling:\nRegion live-ins:";
               OPURPTracker::printLiveRegs(dbgs(), LiveIns[RegionIdx], MRI);
  ...
  OPUMaxOccupancySchedStrategy &S = (OPUMaxOccupancySchedStrategy &)*SchedImpl;
...
OPURegPressure OPUScheduleDAGMILive::getRealRegPressure() const {
  OPUDownwardRPTracker RPTracker(*LIS);
  RPTracker.advance(begin(), end(), &LiveIns[RegionIdx]);
  return RPTracker.moveMaxPressure();
}

void OPUScheduleDAGMILive::computeBlockPressure(const MachineBasicBlock *MBB) {
  OPUDownwardRPTracker RPTracker(*LIS);
...
void OPUScheduleDAGMILive::finalizeSchedule() {
  OPUMaxOccupancySchedStrategy &S = (OPUMaxOccupancySchedStrategy &)*SchedImpl;
