//===- OPUPerfHintAnalysis.cpp - analysis of functions memory traffic --===//
//
...
char llvm::OPUPerfHintAnalysis::ID = 0;
char &llvm::OPUPerfHintAnalysisID = OPUPerfHintAnalysis::ID;

INITIALIZE_PASS(OPUPerfHintAnalysis, DEBUG_TYPE,
                "Analysis if a function is memory bound", true, true)

namespace {

struct OPUPerfHint {
  friend OPUPerfHintAnalysis;

public:
  OPUPerfHint(OPUPerfHintAnalysis::FuncInfoMap &FIM_,
...
private:
  ...
  OPUPerfHintAnalysis::FuncInfoMap &FIM;

  const DataLayout *DL;

  const TargetLowering *TLI;

  OPUPerfHintAnalysis::FuncInfo *visit(const Function &F);
  static bool isMemBound(const OPUPerfHintAnalysis::FuncInfo &F);
  static bool needLimitWave(const OPUPerfHintAnalysis::FuncInfo &F);
...
bool OPUPerfHint::isIndirectAccess(const Instruction *Inst) const {
...
OPUPerfHintAnalysis::FuncInfo *OPUPerfHint::visit(const Function &F) {
  OPUPerfHintAnalysis::FuncInfo &FI = FIM[&F];

  LLVM_DEBUG(dbgs() << "[OPUPerfHint] process " << F.getName() << '\n');
...
bool OPUPerfHint::runOnFunction(Function &F) {
  ...
  const OPUPerfHintAnalysis::FuncInfo *Info = visit(F);
  ...
  if (OPU::isEntryFunctionCC(F.getCallingConv()) && needLimitWave(*Info)) {
...
bool OPUPerfHint::isMemBound(const OPUPerfHintAnalysis::FuncInfo &FI) {
  return FI.MemInstCount * 100 / FI.InstCount > MemBoundThresh;
}

bool OPUPerfHint::needLimitWave(const OPUPerfHintAnalysis::FuncInfo &FI) {
  return ((FI.MemInstCount + FI.IAMInstCount * IAWeight +
           FI.LSMInstCount * LSWeight) *
          100 / FI.InstCount) > LimitWaveThresh;
}

bool OPUPerfHint::isGlobalAddr(const Value *V) const {
...
bool OPUPerfHint::isLocalAddr(const Value *V) const {
  if (auto PT = dyn_cast<PointerType>(V->getType()))
    return PT->getAddressSpace() == AMDGPUAS::LOCAL_ADDRESS;
  return false;
}

bool OPUPerfHint::isLargeStride(const Instruction *Inst) {
...
OPUPerfHint::MemAccessInfo
OPUPerfHint::makeMemAccessInfo(Instruction *Inst) const {
  MemAccessInfo MAI;
...
bool OPUPerfHint::isConstantAddr(const Value *V) const {
  if (auto PT = dyn_cast<PointerType>(V->getType())) {
    unsigned As = PT->getAddressSpace();
...
bool OPUPerfHint::MemAccessInfo::isLargeStride(
...
bool OPUPerfHintAnalysis::runOnSCC(CallGraphSCC &SCC) {
  ...
  for (CallGraphNode *I : SCC) {
    ...
    OPUPerfHint Analyzer(FIM, ST->getTargetLowering());
...
bool OPUPerfHintAnalysis::isMemoryBound(const Function *F) const {
  auto FI = FIM.find(F);
  if (FI == FIM.end())
    return false;

  return OPUPerfHint::isMemBound(FI->second);
}

bool OPUPerfHintAnalysis::needsWaveLimiter(const Function *F) const {
  auto FI = FIM.find(F);
  if (FI == FIM.end())
    return false;

  return OPUPerfHint::needLimitWave(FI->second);
}
