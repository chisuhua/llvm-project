...
// library calls on the OPU target.
//
...
#include "llvm/Transforms/Utils/OPUEmitPrintf.h"
...
#define DEBUG_TYPE "opu-emit-printf"
...
static Value *callPrintfBegin(IRBuilder<> &Builder, ArrayRef<Value *> Args) {
  auto Int64Ty = Builder.getInt64Ty();
  auto Int32Ty = Builder.getInt32Ty();
  auto M = Builder.GetInsertBlock()->getModule();

  Value* Entry_size_Value = Builder.CreateMul(Builder.getInt32(Args.size()),
          Builder.getInt32(8));

  for (unsigned i = 0; i < Args.size(); i++) {
      if (isCString(Args[i])) {
          auto Arglen = Builder.CreateAdd(getStrlenWithNull(Builder, Args[i]), Builder.getInt32(7));
          Arglen = Builder.CreateLShr(Arglen, 3);
          Arglen = Builder.CreateShl(Arglen, 3);
          Entry_size_Value = Builder.CreateAdd(Entry_size_Value, Arglen);
      } else {
          Entry_size_Value = Builder.CreateAdd(Entry_size_Value, Builder.getInt32(8));
      }
  }

  auto Fn = M->getOrInsertFunction("__opu_printf_begin", Int64Ty, Int32Ty);
  return Builder.CreateCall(Fn, {Entry_size_value});
}

static Value *callAppendArgs(IRBuilder<> &Builder, Value *Desc, int NumArgs,
                             Value *Arg0, bool IsLast) {
  auto Int64Ty = Builder.getInt64Ty();
  auto Int32Ty = Builder.getInt32Ty();
  auto M = Builder.GetInsertBlock()->getModule();
  auto Fn = M->getOrInsertFunction("__opu_printf_append_args", Int64Ty,
                                   Int64Ty, Int32Ty, Int64Ty, Int32Ty);
  auto IsLastValue = Builder.getInt32(IsLast);
  auto NumArgsValue = Builder.getInt32(NumArgs);
  return Builder.CreateCall(Fn, {Desc, NumArgsValue, Arg0, IsLastValue});
}
...
static Value *appendArg(IRBuilder<> &Builder, Value *Desc, Value *Arg,
  ...
  return callAppendArgs(Builder, Desc, 1, Arg0, IsLast);
}
...
static Value *getStrlenWithNull(IRBuilder<> &Builder, Value *Str) {
  ...
  // __opu_printf_append_string_n ignores the length if the pointer is null.
  ...
  auto Begin = Builder.CreatePtrToInt(Str, Int32Ty);
  auto End = Builder.CreatePtrToInt(PtrPhi, Int32Ty);
...
static Value *callAppendStringN(IRBuilder<> &Builder, Value *Desc, Value *Str,
  ...
  auto Int32Ty = Builder.getInt32Ty();
  auto M = Builder.GetInsertBlock()->getModule();
  auto Fn = M->getOrInsertFunction("__opu_printf_append_string_n", Int64Ty,
                                   Int64Ty, CharPtrTy, Int32Ty, Int32Ty);
...
Value *llvm::emitOPUPrintfCall(IRBuilder<> &Builder,
                                  ArrayRef<Value *> Args) {
  ...
  auto Desc = callPrintfBegin(Builder, Args);
  ...
  return llvm::ConstantInt::get(Builder.getInt32Ty(), NumOps -1);
}
